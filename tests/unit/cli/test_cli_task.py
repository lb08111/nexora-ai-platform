# -*- coding: utf-8 -*-
"""Tests for the ``jotaduo task`` headless CLI command."""
...
from jotaduo.cli.task_cmd import _read_instruction
...
def test_task_rejects_empty_instruction(monkeypatch) -> None:
    monkeypatch.setattr(
        "jotaduo.config.config.load_agent_config",
        MagicMock(),
    )
...
def test_model_flag_overrides_agent_config(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "jotaduo.cli.task_cmd._run_task",
        AsyncMock(
...
def test_output_dir_writes_result_json(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("jotaduo.cli.task_cmd._run_task", _fake_run_task)
...
async def test_tool_guard_not_bypassed_without_flag():
    ...
    with patch(
        ...
        with patch("jotaduo.app.approvals.get_approval_service"):
...
def test_e2e_cli_no_guard_and_skills_dir(monkeypatch, tmp_path):
    ...
    async def _spy_run_task(**kwargs):
        ...
        captured["env_tool_guard"] = os.environ.get(
            "JOTADUO_TOOL_GUARD_ENABLED",
        )
        captured["env_skills_dir"] = os.environ.get("JOTADUO_SKILLS_DIR")
    ...
    monkeypatch.setattr("jotaduo.cli.task_cmd._run_task", _spy_run_task)
...
def test_isolated_workspace_creates_overlay(tmp_path):
    ...
    from jotaduo.cli.task_cmd import _isolated_skills_workspace
    from jotaduo.agents.skill_system import resolve_effective_skills
