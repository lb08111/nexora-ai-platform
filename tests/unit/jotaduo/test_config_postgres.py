...
class TestGlobalConfigDualMode:
    ...
    def _reset_cache(self):
        import jotaduo.config.utils as utils_mod
    ...
    def test_load_config_uses_pg_when_version_available(self):
        ...
        with (
            mock.patch(
                "jotaduo.config.utils._get_pg_config_version",
                return_value=42,
            ),
            mock.patch(
                "jotaduo.config.utils._load_config_from_pg",
    ...
    def test_save_config_writes_to_pg_and_file(self, tmp_path):
        self._reset_cache()
        from jotaduo.config.config import Config
        from jotaduo.config.utils import save_config

        config_file = tmp_path / "config.json"
        cfg = Config(user_timezone="Europe/London")

        with mock.patch(
            "jotaduo.config.utils._save_config_to_pg",
            return_value=True,
        ) as pg_save:
...
class TestAgentConfigDualMode:
    ...
    def test_load_agent_config_uses_pg_when_available(self, tmp_path):
        ...
        with (
            mock.patch(
                "jotaduo.config.utils.load_config",
                return_value=cfg_obj,
            ),
            mock.patch(
                "jotaduo.config.config._get_pg_agent_config_version",
                return_value=55,
            ),
            mock.patch(
                "jotaduo.config.config._load_agent_config_from_pg",
                return_value=pg_agent,
            ),
        ):
            from jotaduo.config.config import load_agent_config
    ...
    def test_save_agent_config_writes_to_pg_and_file(self, tmp_path):
        ...
        with (
            ...
            mock.patch(
                "jotaduo.config.config._save_agent_config_to_pg",
                return_value=True,
            ) as pg_save,
        ):
            from jotaduo.config.config import save_agent_config
...
class TestRuntimeConfigMigration:
    ...
    def test_migrate_runtime_config(self, tmp_path):
        ...
        from jotaduo_migrate_files_to_postgres import _migrate_runtime_config

        working_dir = tmp_path / "jotaduo"
        working_dir.mkdir()
        ...
        with (
            mock.patch(
                "jotaduo_ext.jotaduo.repositories.config_postgres.save_global_config",
                side_effect=fake_save_global,
            ),
            mock.patch(
                "jotaduo_ext.jotaduo.repositories.config_postgres.save_agent_config",
