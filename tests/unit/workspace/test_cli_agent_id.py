...
from jotaduo.cli.channels_cmd import channels_group
from jotaduo.cli.cron_cmd import cron_group
from jotaduo.cli.daemon_cmd import daemon_group
from jotaduo.cli.chats_cmd import chats_group
from jotaduo.cli.skills_cmd import skills_group
from jotaduo.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test jotaduo channels list uses default agent by default."""
    ...
    with patch("jotaduo.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test jotaduo channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test jotaduo cron list with --agent-id."""
    runner = CliRunner()

    with patch("jotaduo.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test jotaduo daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("jotaduo.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("jotaduo.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test jotaduo daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test jotaduo skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "jotaduo.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("jotaduo.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test jotaduo skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test jotaduo chats list with --agent-id."""
    runner = CliRunner()

    with patch("jotaduo.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
