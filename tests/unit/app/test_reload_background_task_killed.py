...
``Workspace.task_tracker`` (JotaDuo's internal ``TaskTracker``).  Background
...
async def test_unregistered_task_causes_immediate_stop():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # JotaDuo TaskTracker is empty (no registered tasks)
...
async def test_internal_tracked_task_delays_shutdown():
    """Verify that tasks tracked by JotaDuo's ``TaskTracker`` (internal
