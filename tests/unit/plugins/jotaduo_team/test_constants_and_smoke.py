...
def test_orchestrator_uses_meeting_skill():
    ...
    assert "jotaduo-meeting-pt" in orch["skill_names"]
...
def test_skill_md_exists_and_has_frontmatter():
    skill_dir = PLUGIN_DIR / "skills" / "jotaduo-meeting-pt"
    skill_md = skill_dir / "SKILL.md"
    assert skill_md.exists(), f"missing: {skill_md}"
    content = skill_md.read_text(encoding="utf-8")
    assert content.startswith("---\n")
    assert "name: jotaduo-meeting" in content
...
def test_plugin_json_metadata():
    ...
    assert pj["id"] == "jotaduo-team"
    assert pj["entry"]["backend"] == "plugin.py"
    assert "br-specialist-agents" in pj["meta"]["features"]


def test_plugin_class_is_importable():
    from jotaduo_team_plugin import JotaduoTeamPlugin

    assert JotaduoTeamPlugin.__name__ == "JotaduoTeamPlugin"
    # Exposes register + lifecycle hooks.
    assert hasattr(JotaduoTeamPlugin, "register")
    assert hasattr(JotaduoTeamPlugin, "_on_startup")
    assert hasattr(JotaduoTeamPlugin, "_on_shutdown")
