...
- Builds the console frontend (`console/`), copies `console/dist` to `src/jotaduo/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `jotaduo:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/jotaduo:v1 --no-cache`.
