# Build JotaDuo backend with PyInstaller for Tauri sidecar (Windows)
...
$VERSION_FILE = "src\jotaduo\__version__.py"
...
Write-Host "JotaDuo PyInstaller Build - Windows" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
$SPEC_FILE = Join-Path $REPO_ROOT "scripts\pack-tauri\jotaduo.spec"
...
# Verify output
$BACKEND_DIR = Join-Path $DIST "pyinstaller\jotaduo-backend"
$BACKEND_EXE = Join-Path $BACKEND_DIR "jotaduo-backend.exe"
$CLI_EXE = Join-Path $BACKEND_DIR "jotaduo.exe"
...
$DEST = Join-Path $BINARIES_DIR "jotaduo-backend"
