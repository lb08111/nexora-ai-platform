...
info()  { printf "${GREEN}[jotaduo]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[jotaduo]${RESET} %s\n" "$*"; }
error() { printf "${RED}[jotaduo]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
JOTADUO_HOME="${JOTADUO_HOME:-$HOME/.jotaduo}"
JOTADUO_VENV="$JOTADUO_HOME/venv"
JOTADUO_BIN="$JOTADUO_HOME/bin"
PYTHON_VERSION="3.12"
JOTADUO_REPO="https://github.com/agentscope-ai/QwenPaw.git"
...
printf "${GREEN}[jotaduo]${RESET} Installing JotaDuo into ${BOLD}%s${RESET}\n" "$JOTADUO_HOME"
...
if [ -d "$JOTADUO_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$JOTADUO_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$JOTADUO_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$JOTADUO_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/jotaduo/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/jotaduo/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/jotaduo/console/"*
    fi
}

## Ensure docs are available in src/jotaduo/docs/ for source installs.
_DOCS_COPIED=0
prepare_docs() {
    local repo_dir="$1"
    local docs_src="$repo_dir/website/public/docs"
    local docs_dest="$repo_dir/src/jotaduo/docs"
...
cleanup_docs() {
    ...
    if [ "$_DOCS_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/jotaduo/docs"
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing JotaDuo from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        prepare_docs "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$JOTADUO_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
        cleanup_docs "$SOURCE_DIR"
    else
        info "Installing JotaDuo from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        ...
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$JOTADUO_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console/cleanup_docs
    fi
else
    PACKAGE="jotaduo"
    if [ -n "$VERSION" ]; then
        PACKAGE="jotaduo==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$JOTADUO_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package jotaduo
fi

# Verify the CLI entry point exists
[ -x "$JOTADUO_VENV/bin/jotaduo" ] || die "Installation failed: jotaduo CLI not found in venv"
info "JotaDuo installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$JOTADUO_VENV/bin/python" -c "import importlib.resources, jotaduo; p=importlib.resources.files('jotaduo')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$JOTADUO_BIN/jotaduo" << 'WRAPPER'
#!/usr/bin/env bash
# JotaDuo CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

JOTADUO_HOME="${JOTADUO_HOME:-$HOME/.jotaduo}"
REAL_BIN="$JOTADUO_HOME/venv/bin/jotaduo"
...
chmod +x "$JOTADUO_BIN/jotaduo"
info "Wrapper created at $JOTADUO_BIN/jotaduo"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.jotaduo/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.jotaduo/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}JotaDuo installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$JOTADUO_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$JOTADUO_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}jotaduo init${RESET}       # first-time setup\n"
printf "  ${BOLD}jotaduo app${RESET}        # start JotaDuo\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}jotaduo uninstall${RESET}\n"
