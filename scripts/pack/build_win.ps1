...
$Archive = Join-Path $Dist "jotaduo-env.zip"
...
$VersionFile = Join-Path $RepoRoot "src\jotaduo\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "jotaduo-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "jotaduo-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "JotaDuo Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var JOTADUO_LOG_LEVEL or default to "info"
if not defined JOTADUO_LOG_LEVEL set "JOTADUO_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\jotaduo_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.jotaduo\config.json" (
  "%~dp0python.exe" -u -m jotaduo init --defaults --accept-security
)
"%~dp0python.exe" -u -m jotaduo desktop --log-level %JOTADUO_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "JotaDuo Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with JOTADUO_LOG_LEVEL)
if not defined JOTADUO_LOG_LEVEL set "JOTADUO_LOG_LEVEL=debug"
...
echo ====================================
echo JotaDuo Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo PYTHONNOUSERSITE: %PYTHONNOUSERSITE%
echo Log Level: %JOTADUO_LOG_LEVEL%
...
echo [Launch] Starting JotaDuo Desktop with log-level=%JOTADUO_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m jotaduo desktop --log-level %JOTADUO_LOG_LEVEL%
echo.
echo [Exit] JotaDuo Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "JotaDuo Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\JotaDuo Desktop.bat"
WshShell.Run Chr(34) & batPath & Chr(34), 0, False
Set WshShell = Nothing
"@ | Set-Content -Path $LauncherVbs -Encoding ASCII

# Create jotaduo.cmd wrapper in env root so "jotaduo" resolves to this
# instead of Scripts\jotaduo.exe whose embedded Python path may be stale
# after conda-pack/unpack.
$JotaduoCmd = Join-Path $EnvRoot "jotaduo.cmd"
@"
@"%~dp0python.exe" -u -m jotaduo %*
"@ | Set-Content -Path $JotaduoCmd -Encoding ASCII
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('jotaduo'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] JOTADUO_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "JotaDuo-Setup-$Version.exe"
