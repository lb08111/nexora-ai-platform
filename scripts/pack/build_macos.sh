...
# Launcher: force packed env; when no TTY log to ~/.jotaduo/desktop.log (no exec so we see errors)
cat > "${APP_DIR}/Contents/MacOS/${APP_NAME}" << 'LAUNCHER'
#!/usr/bin/env bash
ENV_DIR="$(cd "$(dirname "$0")/../Resources/env" && pwd)"
LOG="$HOME/.jotaduo/desktop.log"
unset PYTHONPATH
export PYTHONHOME="$ENV_DIR"
export PYTHONNOUSERSITE=1
export JOTADUO_DESKTOP_APP=1
...
# Log level: env var JOTADUO_LOG_LEVEL or default to "info"
LOG_LEVEL="${JOTADUO_LOG_LEVEL:-info}"

if [ ! -t 2 ]; then
  mkdir -p "$HOME/.jotaduo"
  { echo "=== $(date) JotaDuo starting ==="
  ...
  if [ ! -f "$HOME/.jotaduo/config.json" ]; then
    "$ENV_DIR/bin/python" -u -m jotaduo init --defaults --accept-security
  fi
  echo "Launching python with log-level=$LOG_LEVEL..."
  "$ENV_DIR/bin/python" -u -m jotaduo desktop --log-level "$LOG_LEVEL"
  EXIT=$?
  if [ $EXIT -ge 128 ]; then
    SIG=$((EXIT - 128))
...
exec "$ENV_DIR/bin/python" -u -m jotaduo desktop --log-level "$LOG_LEVEL"
LAUNCHER
...
if [[ -z "${VERSION}" ]]; then
  ...
  VERSION="$("${APP_DIR}/Contents/Resources/env/bin/python" -c \
    "from importlib.metadata import version; print(version('jotaduo'))" 2>/dev/null \
    || echo "0.0.0")"
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>com.jotaduo.desktop</string>
  <key>CFBundleName</key><string>${APP_NAME}</string>
  <key>CFBundleVersion</key><string>${VERSION}</string>
  <key>CFBundleShortVersionString</key><string>${VERSION}</string>
  ${ICON_PLIST}<key>NSHighResolutionCapable</key><true/>
  <key>LSMinimumSystemVersion</key><string>14.0</string>
  <key>NSDesktopFolderUsageDescription</key><string>JotaDuo may access files in your Desktop folder if you use file-related features. You can choose Don'\''t Allow; the app will still run with limited file access.</string>
</dict>
</plist>
INFOPLIST
...
if [[ -n "${CREATE_ZIP}" ]]; then
  ZIP_NAME="${DIST}/JotaDuo-${VERSION}-macOS.zip"
