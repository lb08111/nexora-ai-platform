...
>
> **Расширение через Skills** — Встроенное планирование по расписанию, обработка PDF и Office, дайджест новостей и многое другое; пользовательские навыки подгружаются автоматически, без привязки к одному поставщику. Именно Skills определяют, что умеет JotaDuo.
>
...
>
> **Любые каналы** — DingTalk, Feishu, WeChat, Discord, Telegram и другие. Один JotaDuo, подключайтесь по необходимости.
>
> <details>
> <summary><b>Что можно делать с JotaDuo</b></summary>
>
> <br>
>
...
- [2026-06-11] **Запуск платформы AgentScope** — Бесплатный деплой JotaDuo, обмен плагинами и маркет Skill. [Попробовать →](https://platform.agentscope.io/)
...
- [2026-05-19] **v1.1.8** — Официальная дистрибуция плагинов, JotaDuo Pet, стриминг-карточки DingTalk / Feishu / Telegram. [v1.1.8 Release Notes →](https://qwenpaw.agentscope.io/release-notes#v1.1.8)
...
- [2026-04-12] **CoPaw переименован в JotaDuo** — глубокая интеграция с экосистемой Qwen, миссия не меняется. [v1.0.0 Release Notes →](https://qwenpaw.agentscope.io/release-notes#v1.0.0)
...
> - **🐍 Я не хочу устанавливать Python**: [Десктопное приложение](#десктопное-приложение-beta) или [Установка через скрипт](#установка-через-скрипт) или [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/JotaDuo)
...
Не требуется ручная настройка Python, одна команда автоматически завершает установку. Скрипт автоматически загрузит uv (менеджер пакетов Python), создаст виртуальное окружение, установит JotaDuo и все зависимости (включая Node.js и frontend-ресурсы). Примечание: может не работать в некоторых сетевых окружениях или при корпоративном контроле доступа.
...
>      - Проверьте доступность `uv`: Введите `uv --version` в CMD. Если отображается номер версии, **настройте только путь JotaDuo**. Если появляется сообщение `'uv' не является внутренней или внешней командой, исполняемой программой или пакетным файлом`, настройте оба пути.
...
>      - Путь JotaDuo: Обычно находится в `%USERPROFILE%\.jotaduo\bin`.
...
>   - **Повторно запустите установку**: Откройте новый терминал и снова выполните скрипт установки для завершения установки `JotaDuo`.
>   - **Настройте переменную окружения `JotaDuo`**: Добавьте `%USERPROFILE%\.jotaduo\bin` в системную переменную `Path`.

После установки откройте новый терминал и выполните:

```bash
jotaduo init --defaults   # или: jotaduo init (интерактивный)
jotaduo app
```
...
# Удаление
jotaduo uninstall          # сохраняет конфигурацию и данные
jotaduo uninstall --purge  # удаляет всё
```
...
Затем откройте консоль в браузере: **http://127.0.0.1:8088/**. Конфигурация, память и навыки сохраняются в томе `jotaduo-data`; настройки моделей и API-ключи сохраняются в томе `jotaduo-secrets`; архивы резервных копий сохраняются в томе `jotaduo-backups`. Для передачи API-ключей (например, `DASHSCOPE_API_KEY`) добавьте `-e VAR=value` или `--env-file .env` в команду `docker run`.

> **Подключение к Ollama или другим сервисам на хост-машине из контейнера**
>
> Внутри Docker-контейнера `localhost` указывает на сам контейнер, а не на хост-машину. Если Ollama (или другие модельные сервисы) запущены на хосте, можно обеспечить доступ JotaDuo из контейнера следующими способами:
>
...
>   agentscope/qwenpaw:latest
> ```
> Затем в JotaDuo **Настройки → Модели** измените Base URL на `http://host.docker.internal:<порт>` — например, для Ollama используйте `http://host.docker.internal:11434`, для LM Studio — `http://host.docker.internal:1234/v1`.
>
...
Если вы хотите развернуть JotaDuo на Alibaba Cloud, используйте развертывание в один клик на ECS: откройте [ссылку развертывания JotaDuo на Alibaba Cloud ECS](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884) и следуйте инструкциям на странице. Подробные шаги см. в [Сообщество разработчиков Alibaba Cloud: Развертывание вашего AI-ассистента за 3 минуты](https://developer.aliyun.com/article/1713682).

---

### Вариант 5: Использование ModelScope

**Не хотите локальную установку?** Используйте [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/JotaDuo) для облачной настройки в один клик. Установите свою студию как **непубличную**, иначе другие смогут управлять вашим JotaDuo.
...
Если вы не привыкли использовать командную строку, можете загрузить и использовать десктопную версию JotaDuo без ручной настройки Python-окружения или выполнения команд.
...
- **macOS**: `JotaDuo-<version>-macOS.zip` (рекомендуется для Apple Silicon)
...
Когда вы загружаете приложение JotaDuo для macOS из Releases, macOS может отобразить: *"Apple не может проверить, что 'JotaDuo' не содержит вредоносного ПО"*. Это происходит потому, что приложение не нотаризовано. Вы все равно можете открыть его следующими способами:

- **Открытие через правый клик (рекомендуется)**
  Щелкните правой кнопкой (или Control + щелчок) по приложению JotaDuo → **"Открыть"** → в диалоговом окне снова нажмите **"Открыть"**. Это сообщит Gatekeeper, что вы доверяете приложению; после этого можно запускать его двойным щелчком как обычно.

- **Разрешить в системных настройках**
  Если всё ещё заблокировано, перейдите в **Системные настройки → Конфиденциальность и безопасность**, прокрутите до сообщения типа *"'JotaDuo' заблокирован, так как невозможно проверить разработчика"*, и нажмите **"Всё равно открыть"** или **"Разрешить"**.
...
Если вы используете **облачный API для LLM** (например, Tongyi Qianwen, Gemini, OpenAI), перед началом диалога необходимо настроить API Key. До настройки действительного ключа JotaDuo не будет нормально работать. Подробности см. в [официальной документации](https://qwenpaw.agentscope.io/docs/models#%E9%85%8D%E7%BD%AE%E4%BA%91%E6%8F%90%E4%BE%9B%E5%95%86).

**Способы настройки:**

1. **Консоль (рекомендуется)** — После запуска `jotaduo app` откройте **http://127.0.0.1:8088/** → **Настройки** → **Модели**. Выберите провайдера, введите **API Key** и активируйте этого провайдера и модель.
2. **`jotaduo init`** — При запуске `jotaduo init` вас проведут через настройку LLM-провайдера и API Key. Следуйте подсказкам, выберите провайдера и введите ключ.
...
JotaDuo может полностью запускать LLM на вашей машине без API-ключей и облачных сервисов. Подробности см. в [официальной документации](https://qwenpaw.agentscope.io/docs/models).
...
jotaduo models # выберите загруженную модель
jotaduo app # запустите сервис
```
...
| Тема                                                          | Описание                                        |
| ------------------------------------------------------------- | ----------------------------------------------- |
| [Введение](https://qwenpaw.agentscope.io/docs/intro)            | Что такое JotaDuo и как его использовать         |
...
JotaDuo включает многоуровневые механизмы безопасности для защиты ваших данных и системы:
...
- **Веб-аутентификация** — Необязательная защита входа в консоль. По умолчанию отключена​; установите `JOTADUO_AUTH_ENABLED=true` для включения. Подробнее см. [Веб-аутентификация](https://qwenpaw.agentscope.io/docs/security#Web-Authentication).
...
Поставьте звезду JotaDuo на GitHub, чтобы получать уведомления о новых релизах первыми.
...
| **Приложение JotaDuo**                          | JotaDuo Creator                                                                                             | В работе        |
|                                                 | JotaDuo Insight                                                                                             | В работе        |
...
- **Затем**: Выполните `jotaduo init --defaults`, затем `jotaduo app`.

> **Примечание при обновлении версии:** После выполнения `git pull` для обновления до новой мажорной версии, пересоберите frontend, переустановите Python-пакет (`pip install -e .`), перезапустите `jotaduo app` и очистите кэш браузера (`Ctrl+Shift+R` или `Cmd+Shift+R` на macOS).

---

## Участие в проекте

JotaDuo развивается через открытую совместную работу, и мы приветствуем любые формы участия! Ознакомьтесь с [Дорожной картой](#дорожная-карта) выше (особенно с пунктами, отмеченными как **Ищем участников**), чтобы найти интересующие вас области, и прочитайте [CONTRIBUTING](https://github.com/agentscope-ai/QwenPaw/blob/main/CONTRIBUTING.md), чтобы начать. Мы особенно приветствуем:
...
JotaDuo расшифровывается как **Qwen Personal Agent Workstation** (персональная рабочая станция агентов Qwen), а также воплощает мудрость Qwen и теплоту Paw. Мы хотим, чтобы это был не холодный инструмент, а умная и тёплая «маленькая лапка», всегда готовая прийти на помощь — самый надёжный партнёр в вашей цифровой жизни.
...
JotaDuo собирает **анонимные** данные об использовании при выполнении `jotaduo init`, чтобы помочь нам понять пользовательскую среду и оптимизировать продукт. Данные собираются **один раз на версию** — когда вы обновляете JotaDuo, происходит повторный сбор для отслеживания распределения версий.

**Собираемая информация:**

- Версия JotaDuo (например, 0.0.7)
...
При интерактивном запуске `jotaduo init` вас спросят о согласии. При использовании режима `--defaults` согласие дается автоматически. Запрос появляется только один раз на версию и не влияет на функциональность JotaDuo.

---

## Лицензия

JotaDuo выпускается под лицензией [Apache License 2.0](LICENSE).

---

## Участники

Благодарим всех, кто внес вклад в развитие JotaDuo:
