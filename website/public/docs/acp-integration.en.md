...
JotaDuo supports **ACP (Agent Client Protocol)** in two complementary ways:

1. **JotaDuo using ACP as a Tool**: JotaDuo connects to external ACP runners and uses them as delegated collaborators
2. **JotaDuo as an ACP Server**: external clients connect to JotaDuo over ACP

This page explains both modes and the scenarios each one fits best.

---

## JotaDuo using ACP as a Tool

In this mode, JotaDuo acts as an **ACP client / orchestrator**, connecting to **configured and enabled external ACP runners** and bringing them into the current session as delegated collaboration capabilities.

The actual entry point for this mode is the built-in tool `delegate_external_agent`. It is intended for scenarios where JotaDuo needs to collaborate with other ACP-capable external agent runtimes, such as the built-in examples `opencode`, `qwen_code`, `claude_code`, and `codex`. For more ACP-compatible agents, see the official ACP agent list and integration guide: <https://agentclientprotocol.com/get-started/agents>. In other words, JotaDuo does not directly talk to arbitrary external agents. It talks to runners that have been registered in ACP configuration, and it starts, continues, responds to, and closes delegated collaboration sessions through them.

### What this mode does

In this mode, JotaDuo uses the built-in `delegate_external_agent` tool to:
...
Conceptually, this lets JotaDuo treat an external agent as a collaborative, tool-like capability while keeping JotaDuo as the primary orchestrator of the main conversation.
...
Once the command-line side is ready, you can configure a custom runner in JotaDuo or collaborate with one of the built-in runners directly.
...
When an external ACP runner asks for permission, JotaDuo does **not** decide on the user's behalf.
...
This keeps delegated ACP execution aligned with the same user-controlled safety model used elsewhere in JotaDuo.

### When to use ACP as a Tool

Use this mode when:

- you want JotaDuo to collaborate with another agent runtime
- you have a specialized ACP-compatible external runner for a certain class of tasks
- you want JotaDuo to remain the primary orchestrator while delegating part of the work outward

### ACP Tool vs MCP

ACP as a Tool and MCP solve different problems:

- **MCP** connects JotaDuo to external services and tool servers
- **ACP as a Tool** connects JotaDuo to an external **agent** runtime
...
## JotaDuo as an ACP Server

In this mode, JotaDuo exposes itself as an [Agent Client Protocol (ACP)](https://github.com/agentclientprotocol/python-sdk) compliant agent service over stdio JSON-RPC. External clients, such as [Zed](https://zed.dev), [OpenCode](https://github.com/nicholasgasior/opencode), or any ACP-compatible editor, can connect to JotaDuo via the `jotaduo acp` command and interact with it programmatically.

### Quick Start

```bash
# Start JotaDuo as an ACP agent
jotaduo acp

# Use a specific agent profile
jotaduo acp --agent mybot

# Use a custom workspace directory
jotaduo acp --workspace /path/to/workspace
...
2. **WORKING_DIR config**: read `agents.active_agent` from `config.json` inside `WORKING_DIR` (default `~/.jotaduo`, or `~/.copaw` for legacy installations; overridable via the `JOTADUO_WORKING_DIR` environment variable)
...
| Aspect               | JotaDuo as an ACP Server                      | JotaDuo using ACP as a Tool                            |
| -------------------- | --------------------------------------------- | ------------------------------------------------------ |
| JotaDuo's role       | Server / target agent                         | Client / orchestrator                                  |
| Connection direction | External client connects to JotaDuo           | JotaDuo connects to an external runner                 |
| Main purpose         | Let editors or external clients drive JotaDuo | Let JotaDuo delegate work to another agent             |
| Typical entry point  | `jotaduo acp`                                 | Delegation tool + ACP runner configuration             |
...
ACP in JotaDuo is not just one feature. It supports both directions:

- **Expose JotaDuo outward** as an ACP server
- **Reach outward from JotaDuo** to external ACP agents as delegated tools

If you are integrating JotaDuo into another client, start with **ACP Server**.
If you want JotaDuo to coordinate with another agent runtime, use **ACP as a Tool**.
