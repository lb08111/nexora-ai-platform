...
The **Console** is JotaDuo's built-in web interface. After running `jotaduo app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with JotaDuo in real time
- Enable/disable/configure messaging channels
- View and manage all chat sessions
- Manage scheduled jobs and heartbeat
- Edit JotaDuo's persona and behavior files
- Enable/import skills to extend JotaDuo's capabilities
...
This is where you talk to JotaDuo. It is the default page when the Console opens.
...
button (↑). JotaDuo replies in real time.
...
Create and manage scheduled jobs that JotaDuo runs automatically by time.
...
The **simplest way to create a cron job is to chat directly with JotaDuo** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message JotaDuo on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task type & content** — **Text**: send fixed text from **Message content**.
     **Agent**: fill **Request content**; on each run JotaDuo receives the text
...
tick, send the contents of `HEARTBEAT.md` as a user message to JotaDuo, and
...
Edit files that define JotaDuo's persona and behavior — `SOUL.md`, `AGENTS.md`,
`HEARTBEAT.md`, etc. — directly in the browser.

> **Multi-agent:** Starting from **v0.1.0**, JotaDuo supports **multi-agent** mode.
> You can run multiple independent agents in one JotaDuo instance, each with its own
...
Manage skills that extend JotaDuo (e.g. read PDF, create Word, fetch news). More
...
Manage runtime environment variables needed by JotaDuo tools and skills (e.g.
`TAVILY_API_KEY`).
...
> **Note:** Variable validity is your responsibility. JotaDuo only stores and loads
> values.
>
...
> Data is stored in `~/.jotaduo/token_usage.json`. Override the filename with
> `JOTADUO_TOKEN_USAGE_FILE`. See [Config — Environment variables](./config#environment-variables).
