...
### JotaDuo vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install JotaDuo

JotaDuo supports multiple installation methods. See
...
5. If using the Desktop app (Windows `.exe` or macOS `.zip`), you currently need to uninstall and reinstall:
   - Uninstall JotaDuo on your machine
   - Download the latest build: https://qwenpaw.agentscope.io/downloads
   - Reinstall

After upgrading, restart the service with `jotaduo app`.

If you previously used CoPaw, upgrading to JotaDuo only requires downloading the latest JotaDuo. No extra migration is needed; your configuration, memory, skills, and other data from the CoPaw era continue to work.

### How to initialize and start JotaDuo service
...
with JotaDuo's default port **8088**. This affects all installation methods
...
- JotaDuo fails to start, or browser cannot connect to `http://127.0.0.1:8088/`
...
1. Run `jotaduo app --port 8090` from a terminal instead
...
### APITimeoutError when running JotaDuo in WSL2 (NAT mode)

When running JotaDuo inside WSL2 with NAT networking (especially when a VPN is
...
After this, JotaDuo should be able to communicate with model providers
normally.

### Open-source repository

JotaDuo is open source. Official repository:
...
or JotaDuo GitHub [Releases](https://github.com/agentscope-ai/QwenPaw/releases).
...
You can also use `jotaduo models` for setup, downloads, and switching. See
[CLI → Models and environment variables → jotaduo models](https://qwenpaw.agentscope.io/docs/cli#jotaduo-models).

### How to use JotaDuo-Flash series models

JotaDuo-Flash is a family of models tuned by the JotaDuo team for JotaDuo's core
...
JotaDuo-Flash models are currently open-sourced on
...
All built-in local providers in JotaDuo can be used with JotaDuo-Flash models:

#### JotaDuo Local (llama.cpp)

In the JotaDuo Local model interface, simply choose a JotaDuo-Flash model to
...
> JotaDuo Local is still in beta. Compatibility across different devices and
> runtime stability are still being improved. If you run into issues while
> using it, please open an issue on GitHub.
> If JotaDuo Local does not work properly in your environment, we recommend
> deploying JotaDuo-Flash with Ollama or LM Studio first.

#### Ollama

1. Download a quantized JotaDuo-Flash model from
   ...
   [JotaDuo-Flash-4B-Q4_K_M](https://www.modelscope.cn/models/AgentScope/JotaDuo-Flash-4B-Q4_K_M).

   - Download with ModelScope CLI:

     ```bash
     modelscope download --model AgentScope/JotaDuo-Flash-4B-Q4_K_M README.md --local_dir ./dir
     ```
...
Create a text file named `jotaduo-flash.txt` with the following contents. Replace
`/path/to/your/jotaduo-xxx.gguf` with the absolute path to the `.gguf` file in
the JotaDuo-Flash model repository you downloaded:
...
4. In JotaDuo model settings, choose the Ollama provider, then automatically load
   the model on the Models page.

#### LM Studio

1. Follow step 1 in the Ollama section above to download an appropriate
   quantized JotaDuo-Flash model.
...
4. In JotaDuo model settings, choose the LM Studio provider, then automatically
   load the model on the Models page.

### When using models deployed with Ollama / LM Studio, why can't JotaDuo complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a JotaDuo bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, JotaDuo may show problems such as:
...
- Before running JotaDuo, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running JotaDuo, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, JotaDuo typically needs a
...
The easiest way to create a cron job is to talk to JotaDuo in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the JotaDuo service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask JotaDuo: “Trigger the ‘drink water reminder’ task I just created.”
...
JotaDuo supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/QwenPaw/issues) in the JotaDuo GitHub
