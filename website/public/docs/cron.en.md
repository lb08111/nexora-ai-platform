...
In JotaDuo, scheduled tasks (cron jobs) let the system run actions
...
2. Fill in fields:
   ...
   - **Task type and content**:
     ...
     - **agent**: send `content.text` to JotaDuo and deliver its response.
...
You can also tell JotaDuo directly in your target channel:
...
See [CLI `jotaduo cron` section](./cli#jotaduo-cron). Common commands:

```bash
jotaduo cron list
jotaduo cron create ...
jotaduo cron state <job_id>
jotaduo cron run <job_id>
jotaduo cron pause <job_id>
jotaduo cron resume <job_id>
jotaduo cron delete <job_id>
```
...
Example (ask JotaDuo every 2 hours and deliver the response):
...
JotaDuo uses five-field cron: **minute hour day month weekday** (no seconds).
...
- [CLI `jotaduo cron` section](./cli#jotaduo-cron) — command reference
