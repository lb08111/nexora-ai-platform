# -*- coding: utf-8 -*-
"""
JotaDuo Runtime Config page object.
...
class RuntimeConfigPage(BasePage):
    ...
    PAGE_LOAD_INDICATOR = '.jotaduo-tabs-tab-btn'

    # Tabs
    REACT_TAB = '[data-node-key="reactAgent"] .jotaduo-tabs-tab-btn'
    LLM_RETRY_TAB = '[data-node-key="llmRetry"] .jotaduo-tabs-tab-btn'
    LLM_RATE_LIMITER_TAB = '[data-node-key="llmRateLimiter"] .jotaduo-tabs-tab-btn'
    CONTEXT_COMPACT_TAB = '[data-node-key="lightContext"] .jotaduo-tabs-tab-btn'
    TOOL_RESULT_COMPACT_TAB = '[data-node-key="lightContext"] .jotaduo-tabs-tab-btn'  # Merged into the Context Management tab
    MEMORY_SUMMARY_TAB = '[data-node-key="remeLightMemory"] .jotaduo-tabs-tab-btn'
    EMBEDDING_CONFIG_TAB = '[data-node-key="remeLightMemory"] .jotaduo-tabs-tab-btn'  # Embedding model config merged into the Long-term Memory tab
    TOOL_EXECUTION_LEVEL_TAB = '[data-node-key="toolExecutionLevel"] .jotaduo-tabs-tab-btn'

    # Active panel
    ACTIVE_PANEL = '.jotaduo-tabs-tabpane-active'

    # Language dropdown
    LANGUAGE_SELECT = '.jotaduo-select'

    # Timezone display
    TIMEZONE_DISPLAY = '.jotaduo-select-selection-item'
    ...
    # Save button
    SAVE_BTN = 'button.jotaduo-btn-primary:has-text("保存"), button:has-text("保 存")'
    RESET_BTN = 'button:has-text("重置"), button:has-text("重 置")'

    # Card title
    CARD_TITLE = '.jotaduo-spark-title'

    # Generic form elements
    SWITCH = '.jotaduo-switch'
    INPUT_NUMBER = '.jotaduo-input-number-input'
    SLIDER = '.jotaduo-slider'
    ...
    def switch_to_tab(self, tab_key: str) -> "RuntimeConfigPage":
        ...
        """
        tab_selector = f'[data-node-key="{tab_key}"] .jotaduo-tabs-tab-btn'
    ...
    def get_memory_backend(self) -> str:
        ...
        if select.is_visible():
            selection = select.locator('.jotaduo-select-selection-item').first
    ...
    def select_language(self, language: str) -> "RuntimeConfigPage":
        ...
        # Pick the option
        dropdown = self.page.locator('.jotaduo-select-dropdown:visible').first
        if dropdown.is_visible():
            option = dropdown.locator(f'.jotaduo-select-item-option:has-text("{language}")').first
    ...
    def get_current_language(self) -> str:
        ...
        language_select = self.get_language_select()
        selection_item = language_select.locator('.jotaduo-select-selection-item').first
    ...
    def get_timezone_display(self) -> Locator:
        ...
        if len(selects) >= 2:
            return selects[1]
        return selects[0] if len(selects) > 0 else self.page.locator('.jotaduo-select').last
    ...
    def click_save(self) -> "RuntimeConfigPage":
        ...
        if not save_btn.is_visible():
            ...
            save_btn = self.page.locator('div[class*="footer"] button.jotaduo-btn-primary').first
    ...
    def assert_config_saved(self) -> "RuntimeConfigPage":
        ...
        error_msg = self.page.locator('.jotaduo-message-error')
