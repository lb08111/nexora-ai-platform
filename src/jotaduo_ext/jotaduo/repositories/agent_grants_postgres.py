    ...
    with db.get_engine().begin() as conn:
        rows = (
            conn.execute(
                ...
                FROM jotaduo_agent_user_grants
    ...
    with db.get_engine().begin() as conn:
        row = (
            conn.execute(
                text(
                    """
                INSERT INTO jotaduo_agent_user_grants
...
def revoke_agent_from_user(agent_id: str, username: str) -> bool:
    ...
    with db.get_engine().begin() as conn:
        result = conn.execute(
            text(
                """
                DELETE FROM jotaduo_agent_user_grants
...
def is_user_granted(agent_id: str, username: str) -> bool:
    ...
    with db.get_engine().begin() as conn:
        row = conn.execute(
            text(
                """
                SELECT 1 FROM jotaduo_agent_user_grants
                WHERE agent_id = :agent_id AND username = :username
                """,
            ),
            {"agent_id": agent_id, "username": username},
...
def delete_all_grants() -> None:
    ...
    db.initialize_schema()
    with db.get_engine().begin() as conn:
        conn.execute(text("DELETE FROM jotaduo_agent_user_grants"))
