# -*- coding: utf-8 -*-
"""Database helpers for Jotaduo persistent storage."""
...
DB_URL_ENV = "JOTADUO_DB_URL"


def get_database_url() -> str:
    """Return the configured Jotaduo database URL."""
...
@lru_cache(maxsize=1)
def get_engine() -> Any:
    """Create and cache a SQLAlchemy engine for Jotaduo storage."""
    ...
    return create_engine(
        database_url,
        pool_pre_ping=True,
        pool_size=int(os.environ.get("JOTADUO_DB_POOL_SIZE", "10")),
        max_overflow=int(os.environ.get("JOTADUO_DB_MAX_OVERFLOW", "20")),
        pool_recycle=int(os.environ.get("JOTADUO_DB_POOL_RECYCLE", "1800")),
...
def initialize_schema() -> None:
    """Create first-phase Jotaduo PostgreSQL tables if needed.
    ...
    statements = [
        """
        CREATE TABLE IF NOT EXISTS jotaduo_audit_events (
            id TEXT PRIMARY KEY,
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_approval_requests (
            id TEXT PRIMARY KEY,
            action TEXT NOT NULL,
        ...
        ON jotaduo_audit_events (created_at DESC)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_audit_actor
        ON jotaduo_audit_events (actor)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_audit_action
        ON jotaduo_audit_events (action)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_audit_status
        ON jotaduo_audit_events (status)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_audit_resource
        ON jotaduo_audit_events (resource_type, resource_id)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_approvals_status
        ON jotaduo_approval_requests (status)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_approvals_action
        ON jotaduo_approval_requests (action)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_approvals_created_at
        ON jotaduo_approval_requests (created_at DESC)
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_agent_policies (
            id TEXT PRIMARY KEY,
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_resource_policies (
            id TEXT PRIMARY KEY,
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_approval_policies (
            id TEXT PRIMARY KEY,
        ...
        ON jotaduo_users (status)
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_roles (
            id TEXT PRIMARY KEY,
            name TEXT NOT NULL,
            description TEXT NOT NULL DEFAULT '',
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_user_roles (
        ...
        ON jotaduo_user_roles (role_id)
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_role_permissions (
            role_id TEXT NOT NULL,
        ...
        ON jotaduo_role_permissions (permission)
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_global_config (
        ...
        """,
        """
        CREATE TABLE IF NOT EXISTS jotaduo_agent_configs (
        ...
        ON jotaduo_agent_configs (updated_at)
        """,
        # -- Multi-tenant: agent-user grants --
        """
        CREATE TABLE IF NOT EXISTS jotaduo_agent_user_grants (
        ...
        ON jotaduo_agent_user_grants (username)
        """,
        """
        CREATE INDEX IF NOT EXISTS idx_cj_agent_grants_agent_id
        ON jotaduo_agent_user_grants (agent_id)
        """,
        # -- Per-capability-type approval configuration --
        """
        CREATE TABLE IF NOT EXISTS jotaduo_capability_approval_config (
        ...
        """
        CREATE TABLE IF NOT EXISTS jotaduo_agent_templates (
        ...
        ON jotaduo_agent_templates (name)
        """,
        # -- Token usage per user --
        """
        CREATE TABLE IF NOT EXISTS jotaduo_token_usage (
...
def check_database_health() -> None:
    ...
    try:
        ...
        raise RuntimeError(
            f"Jotaduo PostgreSQL health check failed: {exc}",
        ) from exc
...
def cascade_delete_agent(agent_id: str) -> dict:
    ...
    with engine.begin() as conn:
        tables = [
            (
                "grants",
                "DELETE FROM jotaduo_agent_user_grants WHERE agent_id = :aid",
            ),
            (
                "agent_policies",
                "DELETE FROM jotaduo_agent_policies WHERE agent_id = :aid",
            ),
            (
                "agent_configs",
                "DELETE FROM jotaduo_agent_configs WHERE agent_id = :aid",
            ),
        ]
