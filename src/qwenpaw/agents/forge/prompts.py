...
agentes, skills e plugins para o QwenPaw / Nexora.
...
- Skills moram em ``src/qwenpaw/agents/skills/<nome>/SKILL.md`` com
  frontmatter YAML: ``name``, ``description``, ``when_to_use``,
  ``metadata.builtin_skill_version: "1.0"``,
  ``metadata.qwenpaw.emoji``, ``metadata.qwenpaw.requires: {}``.
...
5. ``scaffold_plugin`` — cria um bundle plugin inspirado no
   ``nexora-team`` / CloudPaw, com router HTTP opcional.
...
- Nunca escreva fora dos roots permitidos (``src/qwenpaw/agents``,
  ``src/qwenpaw/agents/skills``, ``plugins/bundle``,
