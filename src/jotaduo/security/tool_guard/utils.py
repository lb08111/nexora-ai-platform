...
def _load_config_tool_guard():
    ...
    """
    try:
        from jotaduo.config import load_config
...
def resolve_guarded_tools(
    ...
    2) ``JOTADUO_TOOL_GUARD_TOOLS`` env var
    ...
    raw = EnvVarLoader.get_str("JOTADUO_TOOL_GUARD_TOOLS") or None
    if raw is not None:
...
def resolve_denied_tools(
    ...
    Priority:
    1) constructor-provided ``user_defined``
    2) ``JOTADUO_TOOL_GUARD_DENIED_TOOLS`` env var (comma-separated)
    ...
    raw = EnvVarLoader.get_str("JOTADUO_TOOL_GUARD_DENIED_TOOLS") or None
    if raw is not None:
...
def resolve_auto_denied_rules(
    ...
    Priority:
    1) constructor-provided ``user_defined``
    2) ``JOTADUO_TOOL_GUARD_AUTO_DENIED_RULES`` env var (comma-separated)
    ...
    raw = EnvVarLoader.get_str("JOTADUO_TOOL_GUARD_AUTO_DENIED_RULES") or None
    if raw is not None:
