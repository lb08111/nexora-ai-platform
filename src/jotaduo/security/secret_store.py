...
def _keyring_account() -> str:
    ...
    via ``JOTADUO_WORKING_DIR``) would therefore share — and silently
    overwrite — the stable install's master key, leaving the stable install
    unable to decrypt its own secrets.

    Resolution:

    1. Explicit ``JOTADUO_KEYRING_ACCOUNT`` override always wins (useful for
    ...
    relocated = bool(
        EnvVarLoader.get_str("JOTADUO_WORKING_DIR")
        or EnvVarLoader.get_str("JOTADUO_SECRET_DIR"),
    )
    if not relocated:
