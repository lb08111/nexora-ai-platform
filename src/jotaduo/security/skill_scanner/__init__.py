# -*- coding: utf-8 -*-
"""
Skill security scanner for JotaDuo.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``JOTADUO_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = EnvVarLoader.get_str("JOTADUO_SKILL_SCAN_MODE") or None
    if env is not None:
        val = env.lower().strip()
...
_WORKING_DIR_CURRENT_NAME = ".jotaduo"
