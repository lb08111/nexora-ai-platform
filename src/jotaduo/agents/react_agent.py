# -*- coding: utf-8 -*-
"""JotaDuo Agent - Main agent implementation.

This module provides the main JotaDuoAgent class built on ReActAgent,
...
class JotaDuoAgent(CodingModeMixin, ToolGuardMixin, ReActAgent):
    """JotaDuo Agent with integrated tools, skills, and memory management.
    ...
    MRO note
    ~~~~~~~~
    MRO: JotaDuoAgent → CodingModeMixin → ToolGuardMixin → ReActAgent.
    ...
    def __init__(
        ...
        init_kwargs: dict[str, Any] = {
            "name": agent_config.name or "JotaDuo",
            "model": model,
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_jotaduo_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_jotaduo_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _fix_stringified_json_args(tool_call) -> None:
        ...
        if not isinstance(inp, dict):
            return
        for key in JotaDuoAgent._PLAN_JSON_KEYS:
            val = inp.get(key)
    ...
    def _set_formatter_media_strip(self, enabled: bool) -> None:
        ...
        if formatter is None:
            return
        setattr(formatter, "_jotaduo_force_strip_media", enabled)
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += JotaDuoAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append(
            {"type": "text", "text": JotaDuoAgent._ROUND_END_NOTICE},
        )
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("JotaDuoAgent.reply: max_iters=%s", self.max_iters)
