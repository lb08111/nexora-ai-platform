...
def _github_cache_ttl() -> float:
    raw = EnvVarLoader.get_str("JOTADUO_GITHUB_CACHE_TTL", "")
    if raw:
        try:
...
def _hub_http_timeout() -> float:
    raw = EnvVarLoader.get_str("JOTADUO_SKILLS_HUB_HTTP_TIMEOUT", "30")
    try:
...
def _hub_http_retries() -> int:
    raw = EnvVarLoader.get_str("JOTADUO_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = EnvVarLoader.get_str("JOTADUO_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = EnvVarLoader.get_str("JOTADUO_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_search_path() -> str:
    return EnvVarLoader.get_str(
        "JOTADUO_SKILLS_HUB_SEARCH_PATH",
        "/api/v1/search",
    )
...
def _hub_version_path() -> str:
    return EnvVarLoader.get_str(
        "JOTADUO_SKILLS_HUB_VERSION_PATH",
...
def _hub_detail_path() -> str:
    return EnvVarLoader.get_str(
        "JOTADUO_SKILLS_HUB_DETAIL_PATH",
        "/api/v1/skills/{slug}",
    )
...
def _build_async_client() -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        ...
        headers={"User-Agent": "jotaduo-skills-hub/1.0"},
        limits=httpx.Limits(
...
def _extract_aliyun_skill_spec(url: str) -> str | None:
    ...
    Accepts URLs synthesised by `jotaduo.market.providers.aliyun`, of the
...
async def _fetch_bundle_from_aliyun_url(
    ...
    try:
        from jotaduo.market.providers.aliyun import (
