...
_REQUIREMENTS_METADATA_NAMESPACES = ("openclaw", "jotaduo", "clawdbot")
...
def _extract_emoji_from_metadata(metadata: Any) -> str:
    """Extract emoji from metadata.jotaduo.emoji."""
    if not isinstance(metadata, dict):
        return ""
    jotaduo = metadata.get("jotaduo")
    if isinstance(jotaduo, dict):
        return str(jotaduo.get("emoji", "") or "")
    return ""
...
def extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="jotaduo_skill_upload_"))
...
def staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"jotaduo_skill_stage_{skill_name}_"),
    )
