...
# src/jotaduo/agents/forge/_paths.py → repo root is parents[4]
REPO_ROOT = Path(__file__).resolve().parents[4]

# Allowed write roots. Anything outside these is rejected.
ALLOWED_ROOTS: tuple[Path, ...] = (
    REPO_ROOT / "src" / "jotaduo" / "agents",
    REPO_ROOT / "src" / "jotaduo" / "agents" / "skills",
...
def safe_join(rel_path: str | Path) -> Path:
    ...
    Args:
        rel_path: A path expressed relative to the repo root (e.g.
            ``"plugins/bundle/foo"`` or ``"src/jotaduo/agents/foo"``).
