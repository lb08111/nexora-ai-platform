---
name: guidance
description: "Answer user questions about JotaDuo installation and configuration: first locate and read local documentation, then distill the answer; if local information is insufficient, fall back to the official website documentation."
metadata:
...
# JotaDuo Installation and Configuration Q&A Guide

Use this skill when the user asks about **JotaDuo installation, initialization, environment configuration, dependency requirements, or common configuration options**.
...
```bash
DOCS_DIR=$(python3 -c "from jotaduo.constant import DOCS_DIR; print(DOCS_DIR or '')" 2>/dev/null)
```
...
DOCS_DIR=$(find ~/.jotaduo/memory/ -type d -name "docs")
```
...
Run the following script logic to obtain the variable $JOTADUO_ROOT:

```bash
# Get the absolute path of the binary
JOTADUO_PATH=$(which jotaduo 2>/dev/null || whereis jotaduo | awk '{print $2}')

# Logical deduction: if the path contains .jotaduo/bin/jotaduo, the root is three levels up
# Example: /path/to/JotaDuo/.jotaduo/bin/jotaduo -> /path/to/JotaDuo
if [[ "$JOTADUO_PATH" == *".jotaduo/bin/jotaduo" ]]; then
    JOTADUO_ROOT=$(echo "$JOTADUO_PATH" | sed 's/\/\.jotaduo\/bin\/jotaduo//')
else
    # Fallback: try to get the parent of the parent directory
    JOTADUO_ROOT=$(dirname $(dirname "$JOTADUO_PATH") 2>/dev/null || echo ".")
fi

echo "Detected JotaDuo Root: $JOTADUO_ROOT"
```

Verify and list the documentation directory:
Use the derived $JOTADUO_ROOT to locate the documentation:

```bash
# Construct the standard documentation path
DOCS_DIR="$JOTADUO_ROOT/website/public/docs/"
...
if [ -d "$DOCS_DIR" ]; then
    ...
    find "$JOTADUO_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```

**If project documentation does not exist, search the working directory**

If documentation is still not found, search for available documentation content under the jotaduo installation path:
