...
description: Use this skill to proactively send a one-way message to a user/session/channel, usually only when the user explicitly asks to send to a channel/session or when proactive notification is needed. First query sessions with jotaduo chats list, then push with jotaduo channels send.
metadata:
...
### Should Not Use
- If you are simply replying in the current session, **do not use `jotaduo channels send`**
...
```bash
jotaduo chats list --agent-id <your_agent> --channel <channel>
```

You can also filter by user:

```bash
jotaduo chats list --agent-id <your_agent> --user-id <user_id>
```
...
`jotaduo channels send` requires all of the following:
- `--agent-id`
- `--channel`
- `--target-user`
- `--target-session`
...
`jotaduo channels send` only sends — it does not wait for a reply.
...
```bash
jotaduo chats list --agent-id notify_bot --channel feishu
...
```bash
jotaduo chats list --agent-id task_bot --channel console
...
```bash
jotaduo chats list --agent-id analyst_bot --user-id alice
...
If you are replying to the user in the current session, do not use `jotaduo channels send`.
...
```bash
jotaduo chats list --agent-id <your_agent>
```
...
```bash
jotaduo channels list --agent-id <your_agent>
```
...
- **jotaduo agents chat**: sends to another agent, two-way, returns a reply
- **jotaduo channels send**: sends to a user/session/channel, one-way, no reply

**Selection principle**:
- Need to collaborate with another agent → `jotaduo agents chat`
- Need to proactively push a message to a user → `jotaduo channels send`
...
- `--target-user`: target user ID (obtained from `jotaduo chats list`)
- `--target-session`: target session ID (obtained from `jotaduo chats list`)
...
```bash
jotaduo channels -h
jotaduo channels send -h
jotaduo chats -h
jotaduo chats list -h
```
