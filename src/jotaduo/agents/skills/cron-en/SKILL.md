---
name: cron
description: Use this skill only for scheduled or recurring tasks. Manage jobs with jotaduo cron list/create/get/state/pause/resume/delete/run, and always pass --agent-id explicitly.
metadata:
...
All `jotaduo cron` commands **must** include:
...
```bash
# List tasks
jotaduo cron list --agent-id <agent_id>

# View task details
jotaduo cron get <job_id> --agent-id <agent_id>

# View task status
jotaduo cron state <job_id> --agent-id <agent_id>

# Create a task
jotaduo cron create --agent-id <agent_id> ...

# Delete a task
jotaduo cron delete <job_id> --agent-id <agent_id>

# Pause / Resume a task
jotaduo cron pause <job_id> --agent-id <agent_id>
jotaduo cron resume <job_id> --agent-id <agent_id>

# Run an existing task once immediately
jotaduo cron run <job_id> --agent-id <agent_id>
```
...
```bash
jotaduo cron create --agent-id <agent_id> -f job_spec.json
```
...
5. Create the task with jotaduo cron create
...
- Before modifying/pausing/deleting, run `jotaduo cron list --agent-id <agent_id>` first
- To troubleshoot issues, use `jotaduo cron state <job_id> --agent-id <agent_id>`
...
- Before creating, you can run `jotaduo chats list --agent-id <agent_id>` to get valid `target-user` and `target-session`
...
jotaduo cron -h
jotaduo cron list -h
jotaduo cron create -h
jotaduo cron get -h
jotaduo cron state -h
jotaduo cron pause -h
jotaduo cron resume -h
jotaduo cron delete -h
jotaduo cron run -h
```
