...
description: "Use a headed browser to automatically complete DingTalk channel integration for JotaDuo. Applicable when the user mentions DingTalk, developer console, Client ID, Client Secret, bot, Stream mode, binding or configuring a channel. Supports pausing when a login page is detected and resuming after the user logs in."
metadata:
...
This skill automates the creation of a DingTalk application and the binding of a JotaDuo channel using a headed browser.
...
4. If the user provides no custom values, you must first explicitly reply:
   - "All default settings will be used (JotaDuo / Your personal assistant / default images). Proceeding now."
...
2. Fill in the application information (prefer user-customized values, otherwise use defaults):
   - Application name: default `JotaDuo`
...
Path: `~/.jotaduo/config.json`, under `channels.dingtalk`.
...
2. The agent must not proactively modify the `console` configuration or `~/.jotaduo/config.json`.
3. You must instruct the user to fill in the credentials manually using one of the following two methods:
   ...
   - Configuration file: edit the `channels.dingtalk` field in `~/.jotaduo/config.json`
