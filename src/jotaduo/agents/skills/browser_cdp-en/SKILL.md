...
- This **attaches to an external browser**, not a new process launched by JotaDuo
...
### 1. Managed CDP launched by JotaDuo
...
These browsers are launched and owned by JotaDuo. `stop` will:
...
These browsers are not launched by JotaDuo. `stop` will only:
