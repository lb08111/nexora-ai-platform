...
description: Use this skill when another agent's expertise or context is needed, or when the user explicitly asks to involve another agent. First list agents, then use jotaduo agents chat for two-way communication with replies.
metadata:
...
```bash
jotaduo agents chat --background --task-id <task_id>
```
...
|-----------|-------------|---------|
| Simple quick query | Real-time mode | `jotaduo agents chat` |
| Complex task (data analysis, batch processing, etc.) | Background mode | `jotaduo agents chat --background` |
...
2. jotaduo agents list
3. jotaduo agents chat to start a conversation
...
2. jotaduo agents list
3. jotaduo agents chat --background to submit the task
...
`jotaduo agents chat` must include all of the following:
- `--from-agent`
- `--to-agent`
- `--text`
...
Check status with:
  jotaduo agents chat --background --task-id 20802ea3-...
```
...
sleep 30 && jotaduo agents chat --background \
  --task-id 20802ea3-832d-4fb4-86f0-666ad79fcc80
```
...
jotaduo agents chat --background --task-id <id>
```

**Method 2: Timed polling** (if you must wait)
```bash
# Incrementally increasing intervals, fast first then slow
sleep 10 && jotaduo agents chat --background --task-id <id>
sleep 20 && jotaduo agents chat --background --task-id <id>
sleep 30 && jotaduo agents chat --background --task-id <id>
```
...
```bash
jotaduo agents -h
jotaduo agents list -h
jotaduo agents chat -h
```
