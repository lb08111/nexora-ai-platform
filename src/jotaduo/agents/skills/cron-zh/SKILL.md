---
name: cron
description: 仅在需要未来定时执行或周期执行任务时，使用本 skill。使用 jotaduo cron list/create/get/state/pause/resume/delete/run 管理任务，并始终显式传入 --agent-id。
metadata:
...
- 缺少参数时，先问用户再创建
- 修改/暂停/删除前，先 `jotaduo cron list --agent-id <agent_id>`
- 排查问题时，用 `jotaduo cron state <job_id> --agent-id <agent_id>`
