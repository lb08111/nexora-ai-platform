# -*- coding: utf-8 -*-
"""JotaDuo ACP Agent server.

Exposes JotaDuo as an ACP-compliant agent that external clients
...
ACP_ERROR_META_KEY = "jotaduo.error"
...
ACP_AGENT_META_KEY = "jotaduo.agent"
...
def _msg_to_updates(  # pylint: disable=too-many-branches
    ...
    """Convert a JotaDuo Msg into ACP session update(s).
...
class JotaDuoACPAgent(Agent):
    ...
    async def _ensure_workspace(self) -> Any:
        ...
        logger.info(
            "JotaDuo ACP Agent workspace started: agent_id=%s workspace=%s",
            agent_id,
            workspace_dir,
        )
        return runner
...
async def run_jotaduo_agent(
    agent_id: str | None = None,
    workspace_dir: Path | None = None,
) -> None:
    """Entry point: run JotaDuo as an ACP agent over stdio."""
    agent = JotaDuoACPAgent(
        agent_id=agent_id,
