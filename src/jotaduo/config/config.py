...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is JotaDuo. How can I help you?"
...
class SIPChannelConfig(BaseChannelConfig):
    ...
    language: str = "zh-CN"
    welcome_greeting: str = "你好，我是JotaDuo"
...
class WeChatConfig(BaseChannelConfig):
    ...
    bot_token_file:         Path to persist/load the bot_token
                            (default ~/.jotaduo/wechat_bot_token).
...
def build_fallback_agent_profile_config(
    ...
    Used by :func:`load_agent_config` and ``jotaduo doctor fix``
...
def load_agent_config(  # pylint: disable=too-many-branches,too-many-statements
    ...
    with _agent_config_lock:
        ...
        # This keeps JOTADUO_WORKING_DIR effective even if existing agent.json
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default JotaDuo agent",
    ...
    # When JOTADUO_WORKING_DIR is customized, historical data may still exist
    # under "~/.copaw".
