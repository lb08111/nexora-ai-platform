...
def _normalize_working_dir_bound_paths(data: object) -> object:
    ...
    This keeps JOTADUO_WORKING_DIR effective even if user config files contain
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    jotaduo.channels), filtered by JOTADUO_ENABLED_CHANNELS or
    JOTADUO_DISABLED_CHANNELS when set.

    * JOTADUO_ENABLED_CHANNELS — whitelist (only these channels are active).
    * JOTADUO_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, JOTADUO_ENABLED_CHANNELS takes precedence.
    * If neither is set, all discovered channels are returned.
    ...
    raw_enabled = EnvVarLoader.get_str("JOTADUO_ENABLED_CHANNELS", "").strip()
    if raw_enabled:
...
def is_running_in_container() -> bool:
    ...
    Prefer env JOTADUO_RUNNING_IN_CONTAINER (1/true/yes) at call time so
...
def is_jotaduo_running() -> bool:
    """Check if JotaDuo is currently running by checking API availability.

    Returns:
        True if JotaDuo is running, False otherwise
    """
    try:
