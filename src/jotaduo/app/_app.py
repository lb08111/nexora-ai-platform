...
def _run_jotaduo_startup_migrations() -> None:
    """Keep Jotaduo governance data aligned with configured agents."""
    try:
        ...
        if ordered_ids != config.agents.agent_order:
            ...
            logger.info("Jotaduo normalized agent order for governance")

        from jotaduo_ext.jotaduo.governance import migrate_governance_data

        result = migrate_governance_data(ordered_ids)
        if result.get("changed"):
            logger.info(
                "Jotaduo governance migration applied: %s",
                result,
            )
    except Exception:
        logger.warning(
            "Jotaduo startup migration skipped due to error",
            exc_info=True,
        )
...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    try:
        ...
        message = (
            "JotaDuo startup failed because restore artifact cleanup did not "
    ...
    migrate_legacy_skills_to_skill_pool()
    ensure_qa_agent_exists()
    _run_jotaduo_startup_migrations()

    try:
        from jotaduo_ext.jotaduo.db import check_database_health

        check_database_health()
        logger.info("Jotaduo PostgreSQL health check passed")
    except Exception:
        logger.error(
            "Jotaduo PostgreSQL health check failed — "
...
_CONSOLE_STATIC_ENV = "JOTADUO_CONSOLE_STATIC_DIR"
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get(
        "/{full_path:path}",
        name="jotaduo_console_spa_catchall",
    )
