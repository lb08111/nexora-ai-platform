...
_DEFAULT_AGENT_DESCRIPTION = "Default JotaDuo agent"
...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default JotaDuo agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.jotaduo or WORKING_DIR)
...
def _apply_legacy_qa_disable_for_migration(config) -> None:
    ...
    if getattr(ref, "enabled", True):
        ...
        logger.info(
            ...
            "(new JotaDuo builtin QA slot was created)",
            legacy_id,
        )
...
def ensure_qa_agent_exists() -> None:
    ...
    ``jotaduo_source_index``), and built-in tools are restricted (see
