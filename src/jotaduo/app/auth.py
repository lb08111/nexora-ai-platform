...
variable ``JOTADUO_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``JOTADUO_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = EnvVarLoader.get_str("JOTADUO_AUTH_ENABLED", "").strip().lower()
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``JOTADUO_AUTH_ENABLED``
    is truthy and both ``JOTADUO_AUTH_USERNAME`` and ``JOTADUO_AUTH_PASSWORD``
    ...
    username = EnvVarLoader.get_str("JOTADUO_AUTH_USERNAME", "").strip()
    password = EnvVarLoader.get_str("JOTADUO_AUTH_PASSWORD", "").strip()
...
def _required_permission(request: Request) -> str | None:
    from jotaduo_ext.jotaduo.rbac import required_permission
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if permission:
            from jotaduo_ext.jotaduo.rbac import user_has_permission
    ...
    def _record_request_audit(
        ...
        try:
            from jotaduo_ext.jotaduo.audit import record_audit_event
