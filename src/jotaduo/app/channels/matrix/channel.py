# -*- coding: utf-8 -*-
"""
MatrixChannel: JotaDuo BaseChannel implementation for Matrix (via matrix-nio).
...
logger = logging.getLogger("jotaduo.channels.matrix")
...
# Known JotaDuo slash commands — used to decide whether to strip
# @mention prefix
...
class JotaDuoMatrixClient(AsyncClient):
...
class MatrixChannel(BaseChannel):
    """JotaDuo channel that connects to a Matrix homeserver via matrix-nio."""
    ...
    def __init__(
        ...
        access_token: str = "",
        password: str = "",
        device_name: str = "jotaduo-worker",
        device_id: str = "",
    ...
    def from_config(
        ...
        return cls(
            ...
            device_name=raw.get("device_name", "jotaduo-worker"),
    ...
    def _init_async_client(self, resolved_device_id: str) -> None:
        ...
        self._client = JotaDuoMatrixClient(
            self.homeserver,
    ...
    async def _on_room_event(
        ...
        # Skip history prepend for slash commands — JotaDuo's command parser
    ...
    # native content_parts → JotaDuo Content; same
    # vision_enabled guard as inbound media for image parts (§11).
    # ------------------------------------------------------------------

    # pylint: disable=too-many-return-statements
    def _build_content_part(self, p: dict[str, Any]) -> Any:
        """Convert a native content-part dict to a JotaDuo Content object."""
        t = p.get("type")
    ...
    def build_agent_request_from_native(self, native_payload: Any) -> Any:
        ...
        # in the same room share one session (JotaDuo keys session state on
