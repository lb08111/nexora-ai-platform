...
TELEMETRY_ENDPOINT = (
    "https://jotaduoelemetry-sukzkbfzhc.ap-southeast-1.fcapp.run"
)
...
def _detect_install_method() -> str:
    """Detect how JotaDuo was installed based on environment signals."""
    from ..constant import EnvVarLoader

    if EnvVarLoader.get_bool("JOTADUO_RUNNING_IN_CONTAINER"):
        return "docker"
    if EnvVarLoader.get_bool("JOTADUO_DESKTOP_APP"):
        return "desktop"
    return "pip"
...
def get_system_info() -> dict[str, Any]:
    ...
    - jotaduo_version: JotaDuo version string
    - install_method: How JotaDuo was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running jotaduo (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as jotaduo_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "jotaduo_version": _safe_get(lambda: jotaduo_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current JotaDuo version string."""
    try:
        from ..__version__ import __version__ as jotaduo_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when JotaDuo is upgraded (or downgraded) to a
    version that hasn't been collected before.

    Args:
        working_dir: Path to JotaDuo working directory
    ...
    try:
        ...
        # v1.1 compat: single jotaduo_version field
        return marker_data.get("jotaduo_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("jotaduo_version", "")
                    if old_ver:
