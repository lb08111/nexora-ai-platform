...
def install_sidecar_logging(log_path: Path) -> Path:
    ...
    _LOG_FILE.write("\n")
    _LOG_FILE.write(
        f"== jotaduo tauri sidecar {datetime.now().isoformat()} ==\n",
    )
    ...
    logging.getLogger("jotaduo.tauri").info(
...
def _add_project_file_handler(log_path: Path) -> None:
    # Import lazily so jotaduo.constant is loaded only after desktop env setup.
    from jotaduo.utils.logging import add_project_file_handler
