...
from jotaduo.tauri.env import (
    DESKTOP_APP_ENV,
    DESKTOP_CORS_ORIGINS_ENV,
    DESKTOP_READY_PREFIX,
    ensure_desktop_cors_origins,
)
from jotaduo.tauri.sidecar_logging import install_sidecar_logging

logger = logging.getLogger(__name__)


def _ensure_jotaduo_app_not_loaded() -> None:
    if "jotaduo.app._app" in sys.modules:
        raise RuntimeError(
            "jotaduo app imported before desktop CORS origins were set",
        )


def _sync_loaded_jotaduo_constant_cors_origins() -> None:
    constant_module = sys.modules.get("jotaduo.constant")
...
def _install_desktop_runtime() -> None:
    ...
    # from jotaduo.constant.CORS_ORIGINS at import time.
    _ensure_jotaduo_app_not_loaded()
    ensure_desktop_cors_origins()
    _sync_loaded_jotaduo_constant_cors_origins()
...
def _run_backend_server(log_level: str) -> None:
    ...
    from jotaduo.config.utils import write_last_api
    from jotaduo.constant import LOG_LEVEL_ENV
    from jotaduo.utils.logging import (
    ...
    os.environ.pop("JOTADUO_RELOAD_MODE", None)
    setup_logger(normalized_log_level)
    if normalized_log_level in ("debug", "trace"):
        from jotaduo.cli.main import log_init_timings
...
def main() -> None:
    ...
    from jotaduo.constant import LOG_LEVEL_ENV, WORKING_DIR
    ...
    if not config_path.exists():
        from jotaduo.cli.init_cmd import init_cmd
