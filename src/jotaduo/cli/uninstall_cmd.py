# -*- coding: utf-8 -*-
"""jotaduo uninstall — remove the JotaDuo environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove JotaDuo PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# JotaDuo" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# JotaDuo\nexport PATH=\"\$HOME/\.jotaduo/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove JotaDuo environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL JotaDuo data in {wd}")
    else:
        click.echo(
            "This will remove the JotaDuo Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("JotaDuo uninstalled. Please restart your terminal.")
