...
def _submit_background_task(
    ...
    try:
        ...
        click.echo(f"  jotaduo agents chat --background --task-id {task_id}")
...
def _check_task_status(
    ...
    try:
        ...
        if status == "finished":
            ...
            click.echo(
                f"  jotaduo agents chat --background --task-id {task_id}",
            )
...
def agents_group() -> None:
    ...
    \b
    Examples:
      jotaduo agents chat --from-agent bot_a --to-agent bot_b --text "..."
      jotaduo agents create --name "Research Bot" --agent-id research_bot
      jotaduo agents delete research_bot
      jotaduo agents list
    """
...
def list_agents(ctx: click.Context, base_url: Optional[str]) -> None:
    ...
    Examples:
      jotaduo agents list
      jotaduo agents list --base-url http://192.168.1.100:8088
...
def delete_cmd(
    ...
    \b
    AGENT_ID  Configured agent ID, obtainable via `jotaduo agents list`.

    \b
    Examples:
      jotaduo agents delete research
      jotaduo agents delete research --remove-workspace
      jotaduo agents delete research --yes
    """
...
def chat_cmd(
    ...
    \b
    Prerequisites:
      1. Use 'jotaduo agents list' to discover available agents
      2. Ensure target agent (--to-agent) is configured and running
      3. Use 'jotaduo chats list' to find existing sessions (optional)
