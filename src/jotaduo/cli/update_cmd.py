...
from .process_utils import (
    ...
    _is_jotaduo_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/jotaduo/json"
...
class InstallInfo:
    """Information about the current JotaDuo installation."""
...
class RunningServiceInfo:
    """Detected JotaDuo service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published JotaDuo version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest JotaDuo version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest JotaDuo version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest JotaDuo version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("jotaduo")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running JotaDuo HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local JotaDuo service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_jotaduo_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a JotaDuo HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running JotaDuo service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"JotaDuo service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `jotaduo shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING JOTADUO SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `jotaduo shutdown` will forcibly terminate the current "
        "JotaDuo backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `jotaduo shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `jotaduo shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "jotaduo"]
    ...
    click.echo("")
    click.echo("Running `jotaduo shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `jotaduo shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`jotaduo shutdown` failed. Please stop the running JotaDuo "
            "service manually before running `jotaduo update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"jotaduo=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from jotaduo.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[jotaduo] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[jotaduo] Updating JotaDuo "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[jotaduo] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[jotaduo] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[jotaduo] Update completed successfully.")
        click.echo(
            "[jotaduo] Please restart any running JotaDuo service "
            "to use the new version.",
        )
    else:
        click.echo(f"[jotaduo] Update failed with exit code {return_code}.")
        click.echo(
            "[jotaduo] Please fix the error above and run "
            "`jotaduo update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade JotaDuo in the current Python environment."""
    ...
    if version_check is False:
        click.echo("JotaDuo is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `jotaduo update`, or rerun "
                "without `--yes` to confirm a forced `jotaduo shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                f"Detected {_running_service_display(running)} "
                "after `jotaduo shutdown`. "
                "Please stop it manually before running `jotaduo update`.",
            )

    if not yes and not click.confirm(
        f"Update JotaDuo to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return
