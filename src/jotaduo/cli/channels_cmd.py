...
# Template for `jotaduo channels install <key>` stub (channel key substituted).
...
def configure_wechat(current_config: WeChatConfig) -> WeChatConfig:
    ...
    bot_token_file = click.prompt(
        "bot_token file path",
        default=(
            current_config.bot_token_file or "~/.jotaduo/wechat_bot_token"
        ),
        type=str,
    )
...
def configure_voice(
    ...
    welcome_greeting = click.prompt(
        ...
        or "Hi! This is JotaDuo. How can I help you?",
        type=str,
    )
...
@click.option(
    "--target-user",
    required=True,
    help=("Target user ID (REQUIRED, get from 'jotaduo chats list' query)"),
)
@click.option(
    "--target-session",
    required=True,
    help=("Target session ID (REQUIRED, get from 'jotaduo chats list' query)"),
)
@click.option(
    "--text",
    required=True,
...
def send_cmd(
    ...
    Complete Usage Flow:
      Step 1 - Query available sessions (REQUIRED):
        jotaduo chats list --agent-id my_bot --channel console
      ...
      Step 3 - Send message using queried parameters:
        jotaduo channels send --agent-id my_bot --channel console \\
    ...
    Examples with jq automation:
      ...
      SESSIONS=$(jotaduo chats list --agent-id bot --channel console)
      USER=$(echo "$SESSIONS" | jq -r '.[0].user_id')
      SESSION=$(echo "$SESSIONS" | jq -r '.[0].session_id')

      # Send message
      jotaduo channels send --agent-id bot --channel console \\
        --target-user "$USER" --target-session "$SESSION" \\
        --text "Automated notification"

    \b
    Prerequisites:
      1. MUST use 'jotaduo chats list' to get valid target-user and
         target-session
