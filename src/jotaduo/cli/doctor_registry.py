# -*- coding: utf-8 -*-
"""Extensible hooks for `jotaduo doctor`
(entry points + programmatic registration).

Plugins can expose a setuptools entry point in group ``jotaduo.doctor``::

    [project.entry-points."jotaduo.doctor"]
...
def _load_entry_point_functions() -> list[tuple[str, DoctorNotesFn]]:
    ...
    seen: set[str] = set()
    for group in ("jotaduo.doctor", "copaw.doctor"):
