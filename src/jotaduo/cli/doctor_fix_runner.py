# -*- coding: utf-8 -*-
"""Conservative filesystem repairs for ``jotaduo doctor fix``.
...
``src/jotaduo/console/`` (needs network for npm).

``validate-all-jobs-json`` reuses
:func:`~jotaduo.cli.doctor_checks.check_cron_jobs_files` (read-only); exits
...
from ..utils.console_static import find_jotaduo_source_repo_root
    ...
    *,
        ...
        for fid in fix_ids:
            if fid in RISKY_FIX_IDS and not yes and not dry_run:
                raise ValueError(
                    ...
                    "Use `jotaduo doctor fix --dry-run --only ...` to preview "
                    "the plan without -y.",
                )
    ...
    if (
        ...
        raise ValueError(
            ...
            "ensure-working-dir in --only or run `jotaduo doctor fix` without "
    ...
    if "rebuild-console-npm" in fix_ids:
        ...
        if repo is None:
            skip_msgs.append(
                "rebuild-console-npm: only in a JotaDuo source checkout "
            ...
            def _rebuild_console(
                ...
                if (
                    ...
                    bkp_root = r / ".jotaduo-doctor-fix-backups"
                    sid = _utc_session_id()
                    bkp = bkp_root / sid
