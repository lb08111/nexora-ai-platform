...
def _matches_jotaduo_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a JotaDuo CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m jotaduo {subcommand}",
            f" jotaduo {subcommand}",
            f"__main__.py {subcommand}",
            f'jotaduo.exe" {subcommand}',
            f"jotaduo.exe {subcommand}",
        )
    )


def _is_jotaduo_service_command(command: str) -> bool:
    """Return whether the command line looks like a local JotaDuo app."""
    return _matches_jotaduo_cli_command(command, "app")


def _is_jotaduo_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a JotaDuo CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "jotaduo" or _matches_jotaduo_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local JotaDuo service."""
    if not host:
        return []
