# -*- coding: utf-8 -*-
"""Read-only diagnostics for `jotaduo doctor` (no config or disk mutations)."""
...
# Log file opened on app startup (see ``jotaduo.app._app`` lifespan).
APP_LOG_BASENAME = LOG_FILE_BASENAME

# Built-in local llama.cpp provider id; legacy configs may still use
# copaw-local.
_JOTADUO_LOCAL_PROVIDER_IDS = frozenset({"jotaduo-local", "copaw-local"})
...
def check_app_log_writable() -> tuple[bool, str]:
    ...
    if log_path.exists():
        ...
        return (
            ...
            "(required when starting `jotaduo app`)",
        )
...
def environment_summary_lines(
    ...
    *server_python_environment* describes the **HTTP API process** (running
    ``jotaduo app``), when ``GET /api/doctor/runtime`` returned
...
def windows_environment_lines() -> list[str]:
    """Windows-specific read-only diagnostics for ``jotaduo doctor``."""
...
def legacy_single_agent_workspace_note(cfg: Config) -> str | None:
    ...
    When this applies, ``jotaduo app`` may run an automatic migration;
    ...
    return (
        ...
        "`jotaduo app` uses to trigger legacy → multi-agent workspace "
        "migration. Start `jotaduo app` once (or see docs / `jotaduo init`). "
...
def browser_automation_notes(cfg: Config | None) -> list[str]:
    ...
    use_default = (
        EnvVarLoader.get_str("JOTADUO_BROWSER_USE_DEFAULT", "1")
        .strip()
        .lower()
    )
    if use_default in ("0", "false", "no", "off"):
        notes.append(
            "JOTADUO_BROWSER_USE_DEFAULT is off — browser_use will not "
...
# --- JotaDuo checks (agent.json, channels, MCP, skills, providers) ---
...
def check_enabled_agents_load_agent_config(cfg: Config) -> tuple[bool, str]:
    """Dry-run :func:`~jotaduo.config.config.load_agent_config` for enabled.
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if not path.is_file():
            problems.append(
                ...
                "disk; ensure the file exists or use `jotaduo doctor fix` "
                "where applicable.",
            )
            continue
        try:
...
def jotaduo_local_llm_deep_notes() -> list[str]:
...
async def check_enabled_agents_model_connections(
    ...
    *,
    ...
    for agent_id, ref in cfg.agents.profiles.items():
        ...
        if deep and pid in _JOTADUO_LOCAL_PROVIDER_IDS:
            # Only add once (same underlying llama.cpp runtime).
            if not any("llama.cpp" in n for n in notes):
                notes.extend(jotaduo_local_llm_deep_notes())
...
def console_static_diagnostic_notes() -> list[str]:
    ...
    from ..utils.console_static import (
        CONSOLE_STATIC_ENV,
        find_jotaduo_source_repo_root,
        resolve_console_static_dir,
    )

    notes: list[str] = []
    env_dir = EnvVarLoader.get_str("JOTADUO_CONSOLE_STATIC_DIR", "").strip()
    if env_dir:
        notes.append(
