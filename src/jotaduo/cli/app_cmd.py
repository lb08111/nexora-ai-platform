...
def _warn_if_auth_off_non_loopback_bind(host: str, port: int) -> None:
    """Warn when JotaDuo is reachable beyond loopback without auth."""
    ...
    warning = f"""
============================================================
SECURITY NOTICE: JotaDuo is bound to {bind_address} without authentication.

Anyone who can reach this address may access JotaDuo APIs without login.
...
def app_cmd(
    ...
    if workers is not None:
        ...
        click.echo(
            "   JotaDuo always uses 1 worker for stability. "
    ...
    if reload:
        os.environ["JOTADUO_RELOAD_MODE"] = "1"
    else:
