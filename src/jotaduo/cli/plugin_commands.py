...
from jotaduo.plugins.validation import (
...
def _get_api_base() -> Optional[str]:
    """Return the base URL of the running JotaDuo API, or None.
...
def _api_install_plugin(source: str, force: bool = False) -> bool:
    """Send a hot-install request to the running JotaDuo API.
...
def _api_upload_plugin(zip_path: Path, force: bool = False) -> bool:
    """Send a ZIP file to the running JotaDuo API for hot-install.
...
def _api_uninstall_plugin(plugin_id: str) -> bool:
    """Send a hot-uninstall request to the running JotaDuo API.
...
def _is_running() -> bool:
    """Return whether JotaDuo is currently running.

    Returns:
        ``True`` if the API is reachable, ``False`` otherwise.
    """
    from ..config.utils import is_jotaduo_running

    return is_jotaduo_running()
...
def install(source: str, force: bool):
    ...
    When JotaDuo is running, the plugin is hot-loaded immediately via
    the API (no restart required).  When JotaDuo is stopped, the
    plugin files are copied and will be loaded on next start.

    Examples:
        jotaduo plugin install examples/plugins/idealab-provider
    ...
    if _is_running():
        click.echo(
            "JotaDuo is running — using hot-install via API...",
        )
    ...
    click.echo("\nNext steps:")
    click.echo("   1. Start JotaDuo to load the plugin")
...
def uninstall(plugin_id: str):
    ...
    When JotaDuo is running, the plugin is unloaded immediately via
    the API (no restart required).  When JotaDuo is stopped, only the
    ...
    if _is_running():
        click.echo(
            "JotaDuo is running — using hot-uninstall via API...",
        )
