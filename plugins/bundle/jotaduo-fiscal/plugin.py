# -*- coding: utf-8 -*-
"""Jotaduo Fiscal Plugin for JotaDuo/Jotaduo AI Platform."""
...
logger = logging.getLogger("jotaduo").getChild("plugin.jotaduo-fiscal")
...
def _ensure_default_env_vars() -> None:
    ...
    try:
        ...
        logger.warning("Cannot import jotaduo.envs; env provisioning skipped")
        return
...
def _register_fiscal_tools(api: Any | None = None) -> None:
    ...
    for tool_name in FISCAL_TOOL_NAMES:
        ...
        if callable(register_api):
            try:
                ...
                logger.info(
                    "Registered fiscal tool via PluginApi: %s",
                    tool_name,
                )
    ...
    for tool_name in FISCAL_TOOL_NAMES:
        try:
            ...
            logger.warning(
                "Registry tool registration failed for %s: %s",
                tool_name,
                exc,
            )
...
def _patch_plugin_loader_unload() -> None:
    ...
    if getattr(PluginLoader, "_jotaduo_fiscal_patched", False):
        return
    ...
    async def _patched(
        ...
        if plugin_id == "jotaduo-fiscal":
            logger.info("[jotaduo-fiscal] uninstall detected, cleaning up...")
            try:
                from .agents_setup import uninstall_agents

                uninstall_agents()
            except Exception as exc:  # pylint: disable=broad-except
                logger.warning("[jotaduo-fiscal] cleanup failed: %s", exc)
        return await _original(self, plugin_id, delete_files)

    PluginLoader.unload_plugin = _patched
    PluginLoader._jotaduo_fiscal_patched = True
    logger.info("[jotaduo-fiscal] patched PluginLoader.unload_plugin")


class JotaduoFiscalPlugin:
    """Jotaduo Fiscal plugin entry point."""

    def __init__(self) -> None:
        self._api: Any | None = None

    def register(self, api: Any) -> None:
        logger.info("JotaduoFiscalPlugin.register() called")
        self._api = api
        _init_plugin_path()
        ...
        try:
            ...
            for router, prefix in build_plugin_routers():
                logger.info(
                    "[jotaduo-fiscal] registering router at /api%s",
                    prefix,
                )
        ...
        api.register_startup_hook(
            hook_name="jotaduo_fiscal_init",
            callback=self._on_startup,
            priority=50,
        )
        api.register_shutdown_hook(
            hook_name="jotaduo_fiscal_shutdown",
            callback=self._on_shutdown,
            priority=50,
        )

        _patch_plugin_loader_unload()
        logger.info("[jotaduo-fiscal] hooks registered")

    async def _on_startup(self) -> None:
        from .agents_setup import ensure_builtin_agents

        logger.info("[jotaduo-fiscal] starting up...")
        _ensure_default_env_vars()
        ensure_builtin_agents()
        _register_fiscal_tools(self._api)
        logger.info("[jotaduo-fiscal] startup complete")

    async def _on_shutdown(self) -> None:
        logger.info("[jotaduo-fiscal] shutting down")


plugin = JotaduoFiscalPlugin()
