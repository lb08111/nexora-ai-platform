# -*- coding: utf-8 -*-
"""Jotaduo Pix Plugin for JotaDuo.

Registers the ``jotaduo-cobranca`` agent, exposes Pix billing tools,
...
logger = logging.getLogger("jotaduo").getChild("plugin.jotaduo-pix")
...
def _patch_plugin_loader_unload() -> None:
    ...
    if getattr(PluginLoader, "_jotaduo_pix_patched", False):
        return
    ...
    async def _patched(
        ...
        if plugin_id == "jotaduo-pix":
            logger.info("[jotaduo-pix] uninstall detected, cleaning up...")
            try:
                ...
                logger.warning(
                    "[jotaduo-pix] cleanup failed: %s",
                    exc,
                )
        return await _original(self, plugin_id, delete_files)

    PluginLoader.unload_plugin = _patched
    PluginLoader._jotaduo_pix_patched = True
    logger.info("[jotaduo-pix] patched PluginLoader.unload_plugin")


class JotaduoPixPlugin:
    """Jotaduo Pix plugin entry point."""

    def register(self, api):
        logger.info("JotaduoPixPlugin.register() called")
        _init_plugin_path()
        ...
        try:
            ...
            for router, prefix in build_plugin_routers():
                logger.info(
                    "[jotaduo-pix] registering router at /api%s",
                    prefix,
                )
        ...
        api.register_startup_hook(
            hook_name="jotaduo_pix_init",
            callback=self._on_startup,
            priority=50,
        )
        api.register_shutdown_hook(
            hook_name="jotaduo_pix_shutdown",
            callback=self._on_shutdown,
            priority=50,
        )

        _patch_plugin_loader_unload()
        logger.info("[jotaduo-pix] hooks registered")

    async def _on_startup(self):
        from .agents_setup import ensure_builtin_agents
        from .store.db import init_db

        logger.info("[jotaduo-pix] starting up...")
        _ensure_default_env_vars()
        await init_db()
        ensure_builtin_agents()
        _register_pix_tools()
        logger.info("[jotaduo-pix] startup complete")

    async def _on_shutdown(self):
        logger.info("[jotaduo-pix] shutting down")
