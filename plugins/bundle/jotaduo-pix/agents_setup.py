# -*- coding: utf-8 -*-
"""Agent registration for the jotaduo-pix plugin.

Registers one Brazilian Pix billing agent (``jotaduo-cobranca``) and
...
logger = logging.getLogger("jotaduo").getChild(
    "plugin.jotaduo-pix.agents_setup",
)
...
def ensure_builtin_agents() -> None:
    """Idempotently register the Jotaduo Pix billing agent."""
    try:
        from jotaduo.config.config import (
            AgentProfileConfig,
            AgentProfileRef,
...
def uninstall_agents() -> None:
    """Remove Jotaduo Pix agent profiles and workspaces."""
    try:
        from jotaduo.config.utils import load_config, save_config
    except ImportError:
        return
