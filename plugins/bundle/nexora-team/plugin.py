# -*- coding: utf-8 -*-
"""Nexora Team Plugin for QwenPaw.

Registers the Brazilian specialist team (orchestrator + 4 specialists),
ships the ``nexora-meeting`` skill, exposes HTTP endpoints, and
...
logger = logging.getLogger("qwenpaw").getChild("plugin.nexora-team")
...
def _patch_plugin_loader_unload() -> None:
    ...
    if getattr(PluginLoader, "_nexora_team_patched", False):
        return
    ...
    async def _patched(
        ...
        if plugin_id == "nexora-team":
            logger.info("[nexora-team] uninstall detected, cleaning up...")
            try:
                ...
                logger.warning(
                    "[nexora-team] cleanup failed: %s",
                    exc,
                )
        return await _original(self, plugin_id, delete_files)

    PluginLoader.unload_plugin = _patched
    PluginLoader._nexora_team_patched = True
    logger.info("[nexora-team] patched PluginLoader.unload_plugin")
...
class NexoraTeamPlugin:
    """Nexora Team plugin entry point."""

    def register(self, api):
        logger.info("NexoraTeamPlugin.register() called")
        _init_plugin_path()
        ...
        try:
            ...
            for router, prefix in build_plugin_routers():
                logger.info(
                    "[nexora-team] registering router at /api%s",
                    prefix,
                )
        ...
        api.register_startup_hook(
            hook_name="nexora_team_init",
            callback=self._on_startup,
            priority=50,
        )
        api.register_shutdown_hook(
            hook_name="nexora_team_shutdown",
            callback=self._on_shutdown,
            priority=50,
        )

        _patch_plugin_loader_unload()
        logger.info("[nexora-team] hooks registered")
    ...
    async def _on_startup(self):
        ...
        logger.info("[nexora-team] starting up...")
        _ensure_default_env_vars()
        install_plugin_skills()
        ensure_builtin_agents()
        _register_convene_meeting_tool()
        logger.info("[nexora-team] startup complete")

    async def _on_shutdown(self):
        logger.info("[nexora-team] shutting down")
