# -*- coding: utf-8 -*-
"""Meeting facilitation tool for the Nexora Orchestrator.
...
async def convene_meeting(
    ...
    convener: str = "nexora-orchestrator",
    context: str = "",
    ...
    Args:
        ...
        participants: List of Nexora agent IDs to invite. Defaults to
            all registered Nexora specialists (excluding the convener).
...
def _build_prompt(topic: str, context: str) -> str:
    parts = [
        "Você foi convocado para uma reunião do time Nexora.",
...
async def _call_agent(agent_id: str, prompt: str) -> str:
    """Delegate to QwenPaw's native ``chat_with_agent`` tool.

    Falls back to a stub response (useful in tests) when the platform
    coordinator is not available.
    """
    try:
        from qwenpaw.agents.tools.agent_management import chat_with_agent
    except ImportError:
