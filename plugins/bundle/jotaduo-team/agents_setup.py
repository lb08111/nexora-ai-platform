# -*- coding: utf-8 -*-
"""Agent registration for the jotaduo-team plugin.

Registers five Brazilian specialist agents (orchestrator + 4 staff)
into the JotaDuo multi-agent manager so they appear in ``list_agents``,
the Console UI, and can receive ``chat_with_agent`` calls.

The role-to-prompt mapping reuses ``jotaduo.agents.br_team.prompts``
...
logger = logging.getLogger("jotaduo").getChild(
    "plugin.jotaduo-team.agents_setup",
)
...
def ensure_builtin_agents() -> None:
    """Idempotently register the 5 Jotaduo agents."""
    try:
        from jotaduo.config.config import (
            AgentProfileConfig,
            AgentProfileRef,
...
def _install_workspace_skills(
    ...
    from jotaduo.agents.skill_system import get_skill_pool_dir
...
def uninstall_agents() -> None:
    """Remove Jotaduo agent profiles + workspaces + plugin skills."""
