# -*- coding: utf-8 -*-
"""HTTP API for Jotaduo team meetings.

Endpoints
---------
POST   /api/jotaduo-team/meeting          → convene a new meeting
GET    /api/jotaduo-team/meeting          → list recent meetings
GET    /api/jotaduo-team/meeting/{id}     → fetch a meeting transcript
DELETE /api/jotaduo-team/meeting          → clear the in-memory store
"""
...
logger = logging.getLogger("jotaduo").getChild(
    "plugin.jotaduo-team.routers.meeting",
)

router = APIRouter(prefix="", tags=["jotaduo-team-meeting"])
...
class ConveneMeetingRequest(BaseModel):
    ...
    participants: list[str] | None = Field(
        default=None,
        description=(
            "Agent IDs or bare role names. Default: all 4 Jotaduo "
