# Jotaduo Team — Plugin JotaDuo

> **ID:** `jotaduo-team` · **Versão:** 1.0.0 · **Idioma:** pt-BR
> **Status:** ✅ instalável, com 34 testes verdes.

Plugin que materializa o time de atendentes brasileiros
([`jotaduo.agents.br_team`](../../../src/jotaduo/agents/br_team/)) no
JotaDuo — registra 5 agentes pt-BR prontos para WhatsApp/Pix/Agenda,
adiciona a ferramenta `convene_meeting` para reuniões multi-agente e
expõe uma API HTTP em `/api/jotaduo-team/*`.

## O que ele instala

| Componente | Descrição |
|---|---|
| **5 agentes** | `jotaduo-orchestrator`, `jotaduo-atendente`, `jotaduo-agendamento`, `jotaduo-vendas`, `jotaduo-financeiro` |
| **Skill** | `jotaduo-meeting-pt` (orquestrador convoca reuniões) |
| **Tool** | `convene_meeting(topic, participants, context)` |
| **HTTP API** | `/api/jotaduo-team/team`, `/api/jotaduo-team/meeting` |
| **Env vars** | `WHATSAPP_PROVIDER`, `WHATSAPP_TOKEN`, `PIX_PROVIDER`, `PIX_TOKEN`, `BRASILAPI_BASE_URL` (placeholders no Console) |

## Como instalar

1. Coloque a pasta `jotaduo-team/` em `<jotaduo>/plugins/bundle/`
   (já está, neste repo).
2. Reinicie o backend do JotaDuo — o hook `jotaduo_team_init` roda
   no startup:
...
```bash
curl -X POST http://localhost:8000/api/jotaduo-team/meeting \
...
```json
{
  ...
  "participants": [
    "jotaduo-vendas", "jotaduo-financeiro", "jotaduo-agendamento"
  ],
  "status": "completed",
...
├── plugin.py            ← JotaduoTeamPlugin (register/_on_startup/_on_shutdown)
...
1. **Zero duplicação.** O plugin importa
   `jotaduo.agents.br_team.prompts` para popular a `PERSONA.md` de
...
```powershell
python -m pytest tests/unit/plugins/jotaduo_team/ -q
# 34 passed in 0.7s
```
...
|---|---|---|
| WhatsApp | `src/jotaduo/agents/br_team/tools/whatsapp_tools.py` | Z-API, Evolution, Twilio |
| Pix | `src/jotaduo/agents/br_team/tools/pagamento_tools.py` | Gerencianet, Mercado Pago, Asaas |
| CNPJ/CEP | `src/jotaduo/agents/br_team/tools/cnpj_cep_tools.py` | BrasilAPI (`https://brasilapi.com.br`) |
| Agenda | `src/jotaduo/agents/br_team/tools/agenda_tools.py` | Google Calendar, Outlook, Doctoralia |
...
- **LGPD art. 11** — `jotaduo-recepcionista_saude` (papel disponível
  via `br_team.factory` mas não registrado por padrão neste plugin)
  bloqueia discussão clínica.
- **CDC art. 49** — `jotaduo-suporte` (idem) cita 7 dias de
  arrependimento.
...
- **Reunião** — `convene_meeting` aceita `context` mas o skill
  `jotaduo-meeting-pt` instrui a NUNCA enviar CPF/cartão nesse campo.
...
- [ ] CLI: `jotaduo jotaduo-team meet --topic "..." --with vendas,financeiro`
