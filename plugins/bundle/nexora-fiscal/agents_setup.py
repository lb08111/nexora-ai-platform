# -*- coding: utf-8 -*-
"""Agent registration for the nexora-fiscal plugin."""
...
logger = logging.getLogger("qwenpaw").getChild(
    "plugin.nexora-fiscal.agents_setup",
)
...
def register_extra_tools(agent_id: str, extra_tools: dict[str, dict]) -> None:
    ...
    try:
        ...
        logger.error("Cannot import qwenpaw.config; tool sync skipped")
        return
...
def ensure_builtin_agents() -> None:
    """Idempotently register the Nexora Fiscal specialist agent."""
    try:
        from qwenpaw.config.config import (  # pylint: disable=import-outside-toplevel
            AgentProfileConfig,
            AgentProfileRef,
...
def uninstall_agents() -> None:
    """Remove Nexora Fiscal agent profile and workspace."""
    try:
        from qwenpaw.config.utils import load_config, save_config
    except ImportError:
        return
