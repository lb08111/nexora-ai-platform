# Nexora Fiscal — NFe/NFSe/NFCe BR

> **ID:** `nexora-fiscal` · **Versão:** 1.0.0 · **Idioma:** pt-BR

Plugin fiscal brasileiro para o Nexora AI Platform (fork QwenPaw). Ele registra
...
|---|---|
| Agente | `nexora-fiscal`, papel `fiscal`, persona pt-BR |
...
1. Mantenha a pasta em `plugins/bundle/nexora-fiscal/`.
2. Instale dependências do plugin no ambiente do backend, se necessário:

```powershell
pip install -r plugins\bundle\nexora-fiscal\requirements.txt
```

3. Reinicie o backend do Nexora/QwenPaw.
4. Confira o agente `nexora-fiscal` na área de agentes.
...
curl -X POST http://localhost:8000/api/fiscal/webhook/focus_nfe `
  ...
  -d '{"ref":"nexora-nfe-1","status":"autorizado"}'
```
...
```text
plugins/bundle/nexora-fiscal/
├── plugin.py              # lifecycle, router/tools/agent registration
├── agents_setup.py        # registra o agente nexora-fiscal
