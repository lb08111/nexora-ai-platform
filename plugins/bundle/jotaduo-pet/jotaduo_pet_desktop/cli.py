# -*- coding: utf-8 -*-
"""Command-line management for JotaDuo Pet Desktop."""
...
def command_init(_args: argparse.Namespace) -> int:
    ...
    print(f"JotaDuo working dir: {runtime.jotaduo_working_dir()}")
    ...
    print("")
    print("Install the JotaDuo backend plugin with:")
    print("  jotaduo plugin install ./plugins/jotaduo-pet")
    return 0
...
def command_start(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if health.get("ok"):
            print(f"JotaDuo Pet already running on port {args.port}")
            return 0
    except Exception:
        pass
    ...
    print(f"Started JotaDuo Pet Desktop (pid {process.pid})")
...
def command_switch(args: argparse.Namespace) -> int:
    ...
    if token:
        headers["X-JotaDuo-Pet-Token"] = token
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="jotaduo-pet")
    ...
    g.add_argument(
        "--pet-id",
        help="Folder name under JotaDuo pets/ (e.g. snowpaw)",
    )
    p.add_argument("--port", type=int, default=8765)
