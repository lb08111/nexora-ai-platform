# -*- coding: utf-8 -*-
"""Local HTTP bridge for JotaDuo lifecycle events."""
...
def _token_required() -> bool:
    """Whether the local bridge enforces ``X-JotaDuo-Pet-Token``.
    ...
    setting ``JOTADUO_PET_REQUIRE_TOKEN=0``.
    """
    return os.environ.get("JOTADUO_PET_REQUIRE_TOKEN", "1") != "0"
...
def build_app(
    ...
    app = FastAPI(title="JotaDuo Pet Desktop")
    ...
    async def event(
        payload: PetEvent,
        request: Request,
        x_jotaduo_pet_token: str | None = Header(default=None),
    ) -> dict[str, Any]:
        _check_token(x_jotaduo_pet_token)
