# `jotaduo_pet_desktop` (package internals)

Importable Python package embedded inside the **JotaDuo Pet plugin**.
It ships the desktop runtime: a transparent always-on-top Qt pet
window + a local FastAPI bridge on `127.0.0.1:8765` that JotaDuo (via
...
the package is importable as `jotaduo_pet_desktop` from JotaDuo's
...
| `__main__.py` | `python -m jotaduo_pet_desktop` → delegates to `cli.main`. |
| `cli.py` | CLI subcommands (`python -m jotaduo_pet_desktop ...`): `init`, `start`, `stop`, `status`, `install-pet`, `install-default-pet`, `token`, `switch`, `send`. |
...
| `server.py` | FastAPI app factory (`build_app`). Defines `/health`, `/event`, `/state`, `/bubble`, `/pet`. Optional bearer-style token guard via `JOTADUO_PET_REQUIRE_TOKEN=1` + `X-JotaDuo-Pet-Token`. |
...
| `assets/default-pet/snowpaw/` | The bundled default pet (`pet.json` + `spritesheet.webp`). Loaded via `importlib.resources.files("jotaduo_pet_desktop")`, which resolves relative to this folder on disk (no setuptools install required). |

## Runtime layout

`runtime.home_dir()` defaults to `~/.jotaduo-pet/` (override with
`JOTADUO_PET_HOME`) and contains:
...
~/.jotaduo-pet/runtime/
  ...
  pet-desktop.log    # stdout/stderr of detached `jotaduo-pet start`
```

Pet **packages** live in a separate location resolved by
`runtime.jotaduo_working_dir()`, with this precedence:

1. `JOTADUO_WORKING_DIR`
2. `COPAW_WORKING_DIR`
3. `from jotaduo.constant import WORKING_DIR` (if JotaDuo is installed)
...
send `X-JotaDuo-Pet-Token: <token from runtime/update-token>` on
mutating endpoints (the bundled JotaDuo plugin reads the token file
automatically). To disable the check (e.g. for local development), set
`JOTADUO_PET_REQUIRE_TOKEN=0`.
...
```text
jotaduo.startup   -> waving
jotaduo.shutdown  -> idle
...
These two are equivalent and bypass the `jotaduo-pet start` daemon
wrapper (no PID file, no log redirection — useful while iterating):

```bash
python -m jotaduo_pet_desktop.app --port 8765 --scale 0.58
python -m jotaduo_pet_desktop                 # → cli.main, takes subcommands
```
