...
def home_dir() -> Path:
    return Path(
        os.environ.get(
            "JOTADUO_PET_HOME",
            str(Path.home() / ".jotaduo-pet"),
        ),
    )


def jotaduo_working_dir() -> Path:
    """Return JotaDuo's working directory using JotaDuo's precedence."""
    explicit = os.environ.get("JOTADUO_WORKING_DIR") or os.environ.get(
        "COPAW_WORKING_DIR",
    )
    if explicit:
        return Path(explicit).expanduser().resolve()
    try:
        from jotaduo.constant import WORKING_DIR  # type: ignore
        ...
        return Path("~/.jotaduo").expanduser().resolve()
...
def pets_dir() -> Path:
    return jotaduo_working_dir() / "pets"
...
def current_process_status() -> dict[str, Any]:
    ...
    return {
        ...
        "home": str(home_dir()),
        "jotaduoWorkingDir": str(jotaduo_working_dir()),
