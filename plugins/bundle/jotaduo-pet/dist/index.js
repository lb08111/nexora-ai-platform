const U = "language", _ = "jotaduo-pet-language-change";
function M() {
  try {
...
function pe() {
  const t = "__jotaduoPetLanguageHook", r = Storage.prototype;
  if (r[t]) return;
  const c = r.setItem;
...
const Z = {
  en: {
    routeLabel: "Pet",
    title: "JotaDuo Pet",
    intro: "Installed pets live under your JotaDuo working directory. Start the desktop bridge, then switch the floating pet without restarting JotaDuo.",
    ...
    colAction: "Action",
    switch: "Switch",
    tableEmpty: "No pets found. Run: jotaduo-pet install-pet …",
  ...
  zh: {
    routeLabel: "宠物",
    title: "JotaDuo 桌面宠物",
    intro: "已安装的宠物位于 JotaDuo 工作目录下。启动桌面桥接后，可在不重启 JotaDuo 的情况下切换悬浮宠物。",
    startDesktop: "启动桌面宠物",
    importPet: "导入宠物",
    refresh: "刷新",
    petsDirectory: "宠物目录：",
    ...
    colAction: "操作",
    switch: "切换",
    tableEmpty: "未找到宠物。请运行：jotaduo-pet install-pet …",
...
const R = window.JotaDuo.host, n = R.React, me = R.antd, A = R.getApiUrl, B = R.getApiToken, { Button: L, Card: we, Space: j, Table: he, Typography: ge, message: u, Modal: ye, Checkbox: ke } = me, { Title: Ee, Text: h, Paragraph: Se } = ge;
function be() {
  var t, r, c;
  try {
    const l = ((t = window.sessionStorage) == null ? void 0 : t.getItem("jotaduo-agent-storage")) ?? ((r = window.localStorage) == null ? void 0 : r.getItem("jotaduo-agent-storage"));
    if (!l) return null;
  ...
  return n.useEffect(() => {
    ...
    if (d)
      return (async () => {
        try {
          const b = A(
            `/jotaduo-pet/pets/${encodeURIComponent(t)}/spritesheet`
...
function De() {
  ...
  const te = async () => {
    if (!z) {
      q(!0);
      try {
        const e = await Y("/jotaduo-pet/desktop/start", {}), o = e == null ? void 0 : e.desktop, i = [e == null ? void 0 : e.message, e == null ? void 0 : e.hint].filter(Boolean).join(" ");
    ...
    try {
      ...
      const o = await fetch(A("/jotaduo-pet/import-pet-upload"), {
        method: "POST",
        headers: O(),
        body: e
      }), i = await o.text();
    ...
    const o = e.folder;
    try {
      const i = await Y("/jotaduo-pet/switch-pet", { pet_id: o });
      if (i && i.ok === !1)
...
class Ce {
  ...
  setup() {
    var c, l;
    const r = Q();
    (l = (c = window.JotaDuo).registerRoutes) == null || l.call(c, this.id, [
      {
        path: "/plugin/jotaduo-pet/pets",
        component: De,
