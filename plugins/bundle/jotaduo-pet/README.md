# JotaDuo Pet (plugin + desktop runtime)

Full-stack JotaDuo plugin: backend hooks, console UI sidebar, **and** the
desktop pet runtime itself (`jotaduo_pet_desktop/`). Installing this
plugin gives you everything: the Qt floating pet window, the local HTTP
bridge on `127.0.0.1:8765`, and the JotaDuo side hooks that emit
...
├── router.py              # plugin HTTP routes under /api/jotaduo-pet/...
...
└── jotaduo_pet_desktop/   # the Qt + FastAPI desktop runtime (embedded)
    ...
    ├── cli.py             # `python -m jotaduo_pet_desktop` subcommands
...
time, so `jotaduo_pet_desktop` is importable from JotaDuo's Python
process without any `pip install` step. See
[`jotaduo_pet_desktop/README.md`](jotaduo_pet_desktop/README.md) for
...
- **From the JotaDuo console** — open the plugin management page and
  install `jotaduo-pet` like any other JotaDuo plugin. Three input
  shapes are accepted:
...
- **From the shell** — run:
  ...
  ```bash
  jotaduo plugin install ./plugins/jotaduo-pet
  ```

> [!IMPORTANT]
> **Restart JotaDuo after install** — the backend only picks up new
...
JotaDuo's interpreter needs these packages on its `sys.path` (declared
...
If your JotaDuo install does not auto-resolve plugin dependencies,
install them manually into the same environment:

```bash
pip install -r plugins/jotaduo-pet/requirements.txt
```

PySide6-Essentials wheels exist only for **Python 3.10–3.13**. On 3.14
the pet window cannot start; JotaDuo itself will still run, the pet
...
```python
subprocess.Popen(
    [sys.executable, "-m", "jotaduo_pet_desktop.app",
     "--port", "8765"],
...
python -m jotaduo_pet_desktop.app --port 8765 --scale 0.58

# CLI subcommands (daemonized: PID file + log redirect)
python -m jotaduo_pet_desktop start
python -m jotaduo_pet_desktop status
python -m jotaduo_pet_desktop stop
python -m jotaduo_pet_desktop switch --pet-id snowpaw
...
Autostart can be disabled with `JOTADUO_PET_AUTOSTART=0`.
...
`~/.copaw/plugins/jotaduo-pet/dist/index.js` and refresh the console.
...
host at runtime (`window.JotaDuo.host.React`, `host.antd`,
`host.getApiUrl`, `host.getApiToken`). The shape of this contract is
declared in `frontend/src/jotaduo-host.d.ts`. If the console host
...
The UI adds a sidebar page **Pet** (`/plugin/jotaduo-pet/pets`): lists
pets under `<WORKING_DIR>/pets`, **Start desktop pet** (calls `POST
/api/jotaduo-pet/desktop/start`), **Switch** (hot-switch via `POST
...
`POST /api/jotaduo-pet/import-pet-upload`, then validated and copied
...
## JotaDuo plugin HTTP routes

```text
GET  /api/jotaduo-pet/status
GET  /api/jotaduo-pet/pets
GET  /api/jotaduo-pet/pets/{folder}/spritesheet
POST /api/jotaduo-pet/desktop/start
POST /api/jotaduo-pet/switch-pet
POST /api/jotaduo-pet/import-pet          # JSON body — server-side path
POST /api/jotaduo-pet/import-pet-upload   # multipart/form-data — browser
POST /api/jotaduo-pet/emit-test
```
...
`X-JotaDuo-Pet-Token: <runtime/update-token>` by default — the bundled
JotaDuo plugin reads the token file automatically; standalone clients
must do the same. Set `JOTADUO_PET_REQUIRE_TOKEN=0` to disable the check
...
Row layout (driven by `jotaduo_pet_desktop/sprites.py`):
...
(`jotaduo_pet_desktop/assets/default-pet/snowpaw/`).
...
installed and cached under `~/.jotaduo-pet/cache/snowpaw-spritesheet.webp`
for subsequent runs. Override the source with `JOTADUO_PET_SNOWPAW_URL`
...
`jotaduo_pet_desktop/assets/default-pet/snowpaw/` and the bundled copy
...
- `register_startup_hook` — patches `AgentRunner.query_handler` and the
  ...
  `jotaduo.startup`. Patch failures (e.g. an upstream rename of
  `AgentRunner` / `ApprovalService`) are reported via
  `logger.exception` so a broken plugin install does not stay silently
  dead.
- `register_shutdown_hook` — emits `jotaduo.shutdown`, terminates the
  pet desktop process that this JotaDuo process has adopted (either
  autostarted by the plugin or already healthy on startup /
  `desktop/start`; controlled by `JOTADUO_PET_STOP_ON_SHUTDOWN`), and
  restores the patched class methods. So when the user exits JotaDuo
  the floating pet exits with it, including the case where the pet was
  a leftover from a previous JotaDuo run.
- `register_http_router` — mounts `router.py` under `/jotaduo-pet`.

## Environment variables

| Var | Purpose | Default |
| --- | --- | --- |
| `JOTADUO_PET_DESKTOP_URL` | Full base URL for the plugin → desktop HTTP bridge. If set, port auto-fallback is **disabled** (URL and listener must agree). | unset ⇒ derive from host + port below |
| `JOTADUO_PET_DESKTOP_HOST` | Bind address when spawning (`--host`) | `127.0.0.1` |
| `JOTADUO_PET_DESKTOP_PORT` | Preferred port when spawning; if busy, the plugin scans upward (unless `DESKTOP_URL` is set or `STRICT` is on) | `8765` |
| `JOTADUO_PET_DESKTOP_STRICT_PORT` | `1` ⇒ never auto-pick another port (fail with EADDRINUSE if taken) | `0` |
| `JOTADUO_PET_DESKTOP_SCALE` | Spawn-time scale (e.g. `0.58`) | unset |
| `JOTADUO_PET_DESKTOP_PET_DIR` | Spawn-time pet folder override | unset |
| `JOTADUO_PET_TOKEN_PATH` | Path to the local update token | `~/.jotaduo-pet/runtime/update-token` |
| `JOTADUO_PET_REQUIRE_TOKEN` | `0` ⇒ desktop *skips* the token check on mutating endpoints (anything else, including unset, enforces it) | `1` |
| `JOTADUO_PET_AUTOSTART` | `0` ⇒ plugin will not spawn the desktop | `1` |
| `JOTADUO_PET_STOP_ON_SHUTDOWN` | `0` ⇒ leave the pet desktop running after JotaDuo exits. Default: terminate any pet desktop JotaDuo has adopted (either by autostarting it or by seeing it healthy at startup / explicit `desktop/start`). | `1` |
| `JOTADUO_PET_HOME` | Runtime dir (PID file, log, **cache**, token) | `~/.jotaduo-pet/` |
| `JOTADUO_PET_SNOWPAW_URL` | CDN URL for snowpaw's `spritesheet.webp` (downloaded once on first install) | Alicdn-hosted default |
| `JOTADUO_WORKING_DIR` / `COPAW_WORKING_DIR` | Where `pets/` lives | falls back to `~/.copaw` then `~/.jotaduo` |

### Pet desktop log / common errors

`~/.jotaduo-pet/runtime/pet-desktop.log` captures stderr from the spawned
desktop process.

- **`ModuleNotFoundError: No module named 'PySide6'`** — install Qt into the
  **same** Python as JotaDuo:
...
- **`[Errno 48] address already in use` (uvicorn)** — something else is bound
  ...
  that process, set `JOTADUO_PET_DESKTOP_PORT` to a free port, or unset
  `JOTADUO_PET_DESKTOP_URL` and let the plugin auto-pick the next free port
  after the preferred one.

The effective listen URL is mirrored under
`~/.jotaduo-pet/runtime/desktop-bridge.json` (`url` field) so the plugin can
