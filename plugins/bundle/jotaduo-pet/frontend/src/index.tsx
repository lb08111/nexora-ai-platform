// React and antd are injected by the JotaDuo console host at runtime;
...
//
// ``jotaduo-host.d.ts`` declares the ``window.JotaDuo`` contract so the
...
const host = window.JotaDuo.host;
 ...
 * blob keyed ``jotaduo-agent-storage`` (sessionStorage takes precedence
 * over localStorage so each tab can pin its own agent). The JotaDuo API
...
function getSelectedAgentId(): string | null {
  try {
    const raw =
      window.sessionStorage?.getItem("jotaduo-agent-storage") ??
      window.localStorage?.getItem("jotaduo-agent-storage");
    if (!raw) return null;
...
function PetThumb({ folder }: { folder: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const url = getApiUrl(
          `/jotaduo-pet/pets/${encodeURIComponent(folder)}/spritesheet`,
        );
...
function PetControlPage() {
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      const [petData, st] = await Promise.all([
        apiGet("/jotaduo-pet/pets"),
        apiGet("/jotaduo-pet/status"),
      ]);
  ...
  const startDesktop = async () => {
    ...
    try {
      const r = await apiPost("/jotaduo-pet/desktop/start", {});
      const h = r?.desktop;
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const res = await fetch(getApiUrl("/jotaduo-pet/import-pet-upload"), {
        method: "POST",
        headers: authHeaders(),
        body: form,
      });
  ...
  const switchTo = async (row: PetRow) => {
    ...
    try {
      const r = await apiPost("/jotaduo-pet/switch-pet", { pet_id });
      if (r && r.ok === false) {
...
class JotaDuoPetPlugin {
  readonly id = "jotaduo-pet";

  setup(): void {
    const locale = resolvePetLocale();
    window.JotaDuo.registerRoutes?.(this.id, [
      {
