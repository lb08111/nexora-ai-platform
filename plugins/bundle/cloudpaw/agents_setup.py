...
def _build_acp_config(spec: dict[str, Any]) -> Any:
    ...
    """
    try:
        from jotaduo.config.config import ACPAgentConfig, ACPConfig
    except ImportError:
...
def _inject_llm_env(env: dict[str, str]) -> None:
    ...
    For iac-code >= 0.1.2: write llm_source: jotaduo to settings.yml
    so iac-code reads config directly from JotaDuo.
    ...
    _write_jotaduo_mode_to_settings()
    return


def _write_jotaduo_mode_to_settings() -> None:
    """Write llm_source: jotaduo to ~/.iac-code/settings.yml."""
    import yaml
    ...
    # Write llm_source: jotaduo
    if settings.get("llm_source") != "jotaduo":
        settings["llm_source"] = "jotaduo"
        try:
...
def _initialize_agent_workspace(
    ...
    from jotaduo.agents.skill_system import get_workspace_skills_dir
...
def _seed_persona_md_files(
    ...
    try:
        from jotaduo.agents.prompt import _get_agent_md_dir
...
def _uninstall_plugin_skills() -> None:
    ...
    try:
        from jotaduo.agents.skill_system import (
            get_skill_pool_dir,
...
def _install_workspace_skills(
    ...
    try:
        ...
        from jotaduo.agents.skill_system.store import (
