...
export const multiTenantApi = {
  ...
  listGrantsForAgent: (agentId: string) =>
    request<AgentGrant[]>(
      `/jotaduo/agent-grants/${encodeURIComponent(agentId)}`,
    ),
  listGrantsForUser: (username: string) =>
    request<AgentGrant[]>(
      `/jotaduo/agent-grants/user/${encodeURIComponent(username)}`,
    ),
  ...
  listApprovalConfigs: () =>
    request<CapabilityApprovalConfig[]>("/jotaduo/capability-approval-config"),
  updateApprovalConfig: (
    capType: string,
    payload: Partial<CapabilityApprovalConfig>,
  ) =>
    request<CapabilityApprovalConfig>(
      `/jotaduo/capability-approval-config/${encodeURIComponent(capType)}`,
      {
        method: "PUT",
  ...
  // Agent Templates
  listTemplates: () => request<AgentTemplate[]>("/jotaduo/agent-templates"),
  createTemplate: (
    payload: Omit<AgentTemplate, "builtin" | "created_at" | "updated_at">,
  ) =>
    request<AgentTemplate>("/jotaduo/agent-templates", {
      method: "POST",
      body: JSON.stringify(payload),
    }),
  getTemplate: (templateId: string) =>
    request<AgentTemplate>(
      `/jotaduo/agent-templates/${encodeURIComponent(templateId)}`,
    ),
