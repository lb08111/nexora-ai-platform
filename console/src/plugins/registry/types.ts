/**
 * registry/types.ts — public shapes for the JotaDuo plugin extension registries.
 *
    ...
    dispose() {
      for (const it of d) {
        try {
          ...
          console.warn("[JotaDuo] Disposable threw on dispose:", err);
        }
      }
    },
  };
...
/** A registered route entry (added via builtinRoutes or JotaDuo.route.add). */
