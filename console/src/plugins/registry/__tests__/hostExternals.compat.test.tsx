 ...
 *   window.JotaDuo.registerRoutes("cloudpaw", [{ path: "/a2a", ... }])
 *
...
describe("legacy registerRoutes shim", () => {
  it("attaches window.JotaDuo.{menu, route, slot, audit}", () => {
    expect(typeof window.JotaDuo.menu?.add).toBe("function");
    expect(typeof window.JotaDuo.route?.add).toBe("function");
    expect(typeof window.JotaDuo.slot?.fill).toBe("function");
    expect(typeof window.JotaDuo.audit?.overrides).toBe("function");
  });

  it("translates CloudPaw-shape registerRoutes into menu+route", () => {
    window.JotaDuo.registerRoutes!("cloudpaw", [
      {
        path: "/a2a",
        component: Comp,
        label: "A2A",
  ...
  it("multiple legacy registers share the synthesized plugins-group", () => {
    window.JotaDuo.registerRoutes!("p1", [
      { path: "/p1", component: Comp, label: "P1", icon: "1" },
    ]);
    window.JotaDuo.registerRoutes!("p2", [
  ...
  it("new JotaDuo.menu.add coexists with legacy in plugins-group", () => {
    window.JotaDuo.registerRoutes!("cloudpaw", [
      { path: "/a2a", component: Comp, label: "A2A", icon: "🔗" },
    ]);
    window.JotaDuo.menu!.add("newPlugin", {
      id: "newPlugin.foo",
  ...
  it("legacy registerToolRender still writes to pluginSystem", async () => {
    ...
    const RenderFC = () => null;
    window.JotaDuo.registerToolRender!("p1", { my_tool: RenderFC });
